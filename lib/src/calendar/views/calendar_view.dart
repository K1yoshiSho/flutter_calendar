...
import 'package:workspace_core/core_internal.dart';
...
class _CustomCalendarScrollViewState extends State<CustomCalendarScrollView> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(CustomCalendarScrollView oldWidget) {
    ...
    if ((widget.view == CalendarView.month &&
        ...
        (widget.view != CalendarView.month &&
            (!CalendarViewHelper.isCollectionEqual(widget.calendar.timeSlotViewSettings.nonWorkingDays,
  ...
  Widget build(BuildContext context) {
    ...
    final double viewHeaderHeight =
        isDayView ? 0 : CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view);
    ...
    final double weekNumberPanelWidth = CalendarViewHelper.getWeekNumberPanelWidth(
    ...
    final Widget customScrollWidget = GestureDetector(
      ...
      onHorizontalDragStart: isTimelineView
          ...
          : (DragStartDetails dragStartDetails) {
              _onHorizontalStart(dragStartDetails, isResourceEnabled, isTimelineView, viewHeaderHeight, timeLabelWidth,
                  isNeedDragAndDrop);
            },
      onHorizontalDragUpdate: isTimelineView
          ? null
          : (DragUpdateDetails dragUpdateDetails) {
              _onHorizontalUpdate(dragUpdateDetails, isResourceEnabled, isMonthView, isTimelineView, viewHeaderHeight,
                  timeLabelWidth, resourceItemHeight, weekNumberPanelWidth, isNeedDragAndDrop);
            },
      onHorizontalDragEnd: isTimelineView
          ? null
          : (DragEndDetails dragEndDetails) {
              _onHorizontalEnd(dragEndDetails, isResourceEnabled, isTimelineView, isMonthView, viewHeaderHeight,
    ...
    return GestureDetector(
      onLongPressStart: (LongPressStartDetails details) {
        _handleLongPressStart(
            details, isNeedDragAndDrop, isTimelineView, isResourceEnabled, viewHeaderHeight, timeLabelWidth);
      },
      onLongPressMoveUpdate: isNeedDragAndDrop
          ? (LongPressMoveUpdateDetails details) {
              _handleLongPressMove(details.localPosition, isTimelineView, isResourceEnabled, isMonthView,
                  viewHeaderHeight, timeLabelWidth, resourceItemHeight, weekNumberPanelWidth);
            }
          : null,
      onLongPressEnd: isNeedDragAndDrop
          ? (LongPressEndDetails details) {
              _handleLongPressEnd(details.localPosition, isTimelineView, isResourceEnabled, isMonthView,
                  viewHeaderHeight, timeLabelWidth, weekNumberPanelWidth);
            }
          : null,
      child: Stack(
        children: <Widget>[
                          ...
                          HorizontalDragGestureRecognizer:
                            ...
                            (HorizontalDragGestureRecognizer instance) {
                              ...
                              instance.onStart = (DragStartDetails details) {
                                _handleDragStart(details, isNeedDragAndDrop, isTimelineView, isResourceEnabled,
                                    viewHeaderHeight, timeLabelWidth, resourceViewSize);
                              };
                              instance.onEnd = (DragEndDetails details) {
                                _handleDragEnd(details, isTimelineView, isResourceEnabled, isMonthView,
                                    viewHeaderHeight, timeLabelWidth, weekNumberPanelWidth, isNeedDragAndDrop);
                              };
                  ...
                  child: RepaintBoundary(
                      child: _DraggingAppointmentWidget(
                          ...
                          AppointmentHelper.getAppointmentTextStyle(
                              widget.calendar.appointmentTextStyle, widget.view, widget.themeData),
  ...
  void _handleAppointmentDragStart(AppointmentView appointmentView, bool isTimelineView, Offset details,
      bool isResourceEnabled, double viewHeaderHeight, double timeLabelWidth) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
    currentState._updateDraggingMouseCursor(true);
    _dragDetails.value.timeIntervalHeight = currentState._getTimeIntervalHeight(widget.calendar, widget.view,
        widget.width, widget.height, currentState.widget.visibleDates.length, widget.isMobilePlatform);
    _dragDetails.value.appointmentView = appointmentView;
    _dragDifferenceOffset = null;
    final Offset appointmentPosition = Offset(
        widget.isRTL ? appointmentView.appointmentRect!.right : appointmentView.appointmentRect!.left,
    ...
    if (isTimelineView) {
      ...
      final double allDayHeight =
          currentState._isExpanded ? _updateCalendarStateDetails.allDayPanelHeight : currentState._allDayHeight;
      xPosition = appointmentPosition.dx - details.dx;
      yPosition =
    ...
    _dragDetails.value.draggingTime = yPosition <= 0 && widget.view != CalendarView.month && !isTimelineView
        ? null
    ...
    if (widget.calendar.onDragStart != null) {
      widget.calendar.onDragStart!(AppointmentDragStartDetails(dragStartAppointment, selectedResource));
    }
  }

  void _handleLongPressStart(LongPressStartDetails details, bool isNeedDragAndDrop, bool isTimelineView,
      bool isResourceEnabled, double viewHeaderHeight, double timeLabelWidth) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
    ...
    _handleAppointmentDragStart(
        appointmentView, isTimelineView, details.localPosition, isResourceEnabled, viewHeaderHeight, timeLabelWidth);
  }
  ...
  void _handleLongPressMove(Offset details, bool isTimelineView, bool isResourceEnabled, bool isMonthView,
      double viewHeaderHeight, double timeLabelWidth, double resourceItemHeight, double weekNumberPanelWidth) {
    ...
    final double timeIntervalHeight = currentState._getTimeIntervalHeight(widget.calendar, widget.view, widget.width,
        widget.height, currentState.widget.visibleDates.length, widget.isMobilePlatform);
    if (isTimelineView) {
      ...
      _updateNavigationDayView(currentState, appointmentPosition, viewHeaderHeight, allDayHeight, timeIntervalHeight,
          timeLabelWidth, isResourceEnabled, isTimelineView, isMonthView, details, weekNumberPanelWidth);
    }
  ...
  Future<void> _updateNavigationDayView(
    ...
    if (widget.calendar.dragAndDropSettings.allowScroll &&
      ...
      _timer = Timer(const Duration(milliseconds: 200), () async {
        if (_dragDetails.value.position.value != null &&
            _dragDetails.value.position.value!.dy + _dragDetails.value.appointmentView!.appointmentRect!.height >=
                widget.height &&
        ...
        ((isHorizontalNavigation &&
                (appointmentPosition.dx + _dragDetails.value.appointmentView!.appointmentRect!.width) - rtlValue >=
                    widget.width) ||
            (!isHorizontalNavigation &&
                (appointmentPosition.dy + _dragDetails.value.appointmentView!.appointmentRect!.height >=
                    widget.height)))) {
      if (_timer != null) {
        return;
      }
      ...
      _timer = Timer.periodic(const Duration(seconds: 1), (Timer timer) async {
        if (_dragDetails.value.position.value != null &&
            ((isHorizontalNavigation &&
                    (_dragDetails.value.position.value!.dx + navigationThresholdValue) - rtlValue <= 0) ||
  ...
  Future<void> _updateAutoScrollDragTimelineView(
    ...
    if (widget.calendar.dragAndDropSettings.allowScroll &&
      ...
      _timer = Timer(const Duration(milliseconds: 200), () async {
        if (_dragDetails.value.position.value != null &&
            (_dragDetails.value.position.value!.dx + _dragDetails.value.appointmentView!.appointmentRect!.width) -
                    rtlValue >=
                widget.width &&
            ((widget.isRTL && currentState._scrollController!.position.pixels != 0) ||
                (!widget.isRTL &&
      ...
      if (widget.calendar.dragAndDropSettings.allowScroll &&
        ...
        _timer = Timer(const Duration(milliseconds: 200), () async {
          if (_dragDetails.value.position.value != null &&
            ...
            Future<void> updateScrollPosition() async {
              double scrollPosition =
                  currentState._timelineViewVerticalScrollController!.position.pixels - resourceItemHeight;
              if (scrollPosition < 0) {
                scrollPosition = 0;
              }
              await currentState._timelineViewVerticalScrollController!.position.moveTo(
                scrollPosition,
                duration: const Duration(milliseconds: 100),
                curve: Curves.easeInOut,
              );
          ...
          currentState._timelineViewVerticalScrollController!.position.pixels !=
              currentState._timelineViewVerticalScrollController!.position.maxScrollExtent) {
        if (_timer != null) {
          return;
        }
        _timer = Timer(const Duration(milliseconds: 200), () async {
  ...
  void _updateAutoViewNavigationTimelineView(
    ...
    if (widget.calendar.dragAndDropSettings.allowNavigation &&
        (appointmentPosition.dx + _dragDetails.value.appointmentView!.appointmentRect!.width) - rtlValue >=
            widget.width &&
        ((!widget.isRTL &&
                currentState._scrollController!.offset == currentState._scrollController!.position.maxScrollExtent) ||
  ...
  void _updateAppointmentDragUpdateCallback(
    ...
    final double overAllWidth = isTimelineView
        ? currentState._timeIntervalHeight *
            (currentState._horizontalLinesCount! * currentState.widget.visibleDates.length)
        : widget.width;
    final double overAllHeight = isTimelineView || isMonthView
        ? widget.height
        : currentState._timeIntervalHeight * currentState._horizontalLinesCount!;

    if (isTimelineView &&
        overAllWidth < widget.width &&
        xPosition + _dragDetails.value.appointmentView!.appointmentRect!.width > overAllWidth) {
      xPosition = overAllWidth - _dragDetails.value.appointmentView!.appointmentRect!.width;
        ...
        yPosition + _dragDetails.value.appointmentView!.appointmentRect!.height > overAllHeight) {
      yPosition = overAllHeight - _dragDetails.value.appointmentView!.appointmentRect!.height;
    }

    draggingTime = currentState._getDateFromPosition(xPosition, yPosition, timeLabelWidth)!;
    if (!isMonthView) {
      if (isTimelineView) {
        final DateTime time = _timeFromPosition(draggingTime, widget.calendar.timeSlotViewSettings, xPosition,
            currentState, timeIntervalHeight, isTimelineView)!;
        ...
        if (yPosition < 0) {
          ...
          draggingTime = _timeFromPosition(draggingTime, widget.calendar.timeSlotViewSettings, yPosition, currentState,
              timeIntervalHeight, isTimelineView)!;
        }
      }
    }

    _dragDetails.value.position.value =
        Offset(_dragDetails.value.position.value!.dx, _dragDetails.value.position.value!.dy - 0.1);
    _dragDetails.value.draggingTime =
        yPosition <= 0 && widget.view != CalendarView.month && !isTimelineView ? null : draggingTime;
    _dragDetails.value.position.value =
        Offset(_dragDetails.value.position.value!.dx, _dragDetails.value.position.value!.dy + 0.1);

    final dynamic draggingAppointment =
        _getCalendarAppointmentToObject(_dragDetails.value.appointmentView!.appointment, widget.calendar);
    ...
    if (isResourceEnabled) {
      ...
      sourceSelectedResourceIndex = currentState._getSelectedResourceIndex(
          _dragDetails.value.appointmentView!.appointmentRect!.top, viewHeaderHeight, timeLabelWidth);
    ...
    final Duration appointmentDuration = _dragDetails.value.appointmentView!.appointment!.isAllDay &&
            widget.view != CalendarView.month &&
            !isTimelineView
        ? const Duration(hours: 1)
        : _dragDetails.value.appointmentView!.appointment!.endTime
            .difference(_dragDetails.value.appointmentView!.appointment!.startTime);
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
        ...
        (widget.view == CalendarView.month &&
            !CalendarViewHelper.isCurrentMonthDate(widget.calendar.monthViewSettings.numberOfWeeksInView,
                widget.calendar.monthViewSettings.showTrailingAndLeadingDates, currentMonth, draggingTime))) {
    ...
    if (widget.calendar.onDragUpdate != null) {
      widget.calendar.onDragUpdate!(AppointmentDragUpdateDetails(draggingAppointment, previousResource,
          selectedResource, appointmentPosition, _dragDetails.value.draggingTime));
    }
  }

  void _handleLongPressEnd(Offset details, bool isTimelineView, bool isResourceEnabled, bool isMonthView,
      double viewHeaderHeight, double timeLabelWidth, double weekNumberPanelWidth) {
    if (_dragDetails.value.appointmentView == null) {
      return;
    }
    ...
    DateTime? dropTime = currentState._getDateFromPosition(xPosition, yPosition, timeLabelWidth);
    if (!isMonthView) {
      if (isTimelineView) {
        final DateTime time = _timeFromPosition(dropTime!, widget.calendar.timeSlotViewSettings, xPosition,
            currentState, timeIntervalHeight, isTimelineView)!;
        dropTime = DateTime(dropTime.year, dropTime.month, dropTime.day, time.hour, time.minute);
      } else {
        dropTime = _timeFromPosition(dropTime!, widget.calendar.timeSlotViewSettings, yPosition, currentState,
            timeIntervalHeight, isTimelineView);
      }
    }
    ...
    if (!isTimelineView && widget.view != CalendarView.month) {
      ...
      if (_dragDetails.value.appointmentView!.appointment!.isSpanned && !appointment.isAllDay) {
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
        ...
        (widget.view == CalendarView.month &&
            !CalendarViewHelper.isCurrentMonthDate(widget.calendar.monthViewSettings.numberOfWeeksInView,
                widget.calendar.monthViewSettings.showTrailingAndLeadingDates, currentMonth, dropTime))) {
    ...
    if ((appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) ||
      ...
      final List<DateTime> recurrenceDates = RecurrenceHelper.getRecurrenceDateTimeCollection(
          ...
          recurrenceDuration:
              AppointmentHelper.getDifference(parentAppointment.exactStartTime, parentAppointment.exactEndTime),
      ...
      for (int i = 0; i < _updateCalendarStateDetails.appointments.length; i++) {
        ...
        if (calendarApp.recurrenceId != null && calendarApp.recurrenceId == parentAppointment.id) {
          recurrenceDates.add(AppointmentHelper.convertTimeToAppointmentTimeZone(
              calendarApp.startTime, calendarApp.startTimeZone, widget.calendar.timeZone));
        }
      }

      if (parentAppointment.recurrenceExceptionDates != null) {
        for (int i = 0; i < parentAppointment.recurrenceExceptionDates!.length; i++) {
          recurrenceDates.remove(AppointmentHelper.convertTimeToAppointmentTimeZone(
              parentAppointment.recurrenceExceptionDates![i], '', widget.calendar.timeZone));
        }
      }
      ...
      if (appointment.recurrenceId != null &&
          (appointment.recurrenceRule == null || appointment.recurrenceRule!.isEmpty)) {
        ...
        final DateTime exceptionDate = AppointmentHelper.convertTimeToAppointmentTimeZone(
    ...
    if (widget.calendar.onDragEnd != null) {
      widget.calendar
          .onDragEnd!(AppointmentDragEndDetails(newAppointment, previousResource, selectedResource, callbackStartDate));
    }
  }
  ...
  void _handleDragStart(DragStartDetails details, bool isNeedDragAndDrop, bool isTimelineView, bool isResourceEnabled,
      double viewHeaderHeight, double timeLabelWidth, double resourceViewSize) {
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      return;
    }
    final _CalendarViewState viewKey = _getCurrentViewByVisibleDates()!;
    if (viewKey._hoveringAppointmentView != null && !widget.isMobilePlatform && isNeedDragAndDrop) {
      _handleAppointmentDragStart(
          viewKey._hoveringAppointmentView!.clone(),
          isTimelineView,
          Offset(details.localPosition.dx - widget.width, details.localPosition.dy),
          isResourceEnabled,
          viewHeaderHeight,
          timeLabelWidth);
      return;
    ...
    /// move the previous view to end of the scroll or move the next view to
    /// start of the scroll
    if (_timelineScrollStartPosition >= viewKey._scrollController!.position.maxScrollExtent) {
      _positionTimelineView();
    } else if (_timelineScrollStartPosition <= viewKey._scrollController!.position.minScrollExtent) {
      _positionTimelineView();
    }
  ...
  void _handleDragUpdate(
    ...
    if (_dragDetails.value.appointmentView != null && !widget.isMobilePlatform && isNeedDragAndDrop) {
      _handleLongPressMove(Offset(details.localPosition.dx - widget.width, details.localPosition.dy), isTimelineView,
          isResourceEnabled, isMonthView, viewHeaderHeight, timeLabelWidth, resourceItemHeight, weekNumberPanelWidth);
      return;
    }
  ...
  void _handleDragEnd(DragEndDetails details, bool isTimelineView, bool isResourceEnabled, bool isMonthView,
      double viewHeaderHeight, double timeLabelWidth, double weekNumberPanelWidth, bool isNeedDragAndDrop) {
    if (_dragDetails.value.appointmentView != null && !widget.isMobilePlatform && isNeedDragAndDrop) {
      _handleLongPressEnd(_dragDetails.value.position.value! - _dragDifferenceOffset!, isTimelineView,
          isResourceEnabled, isMonthView, viewHeaderHeight, timeLabelWidth, weekNumberPanelWidth);
      return;
    }
  ...
  void _handlePointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent && viewKey != null) {
      double scrolledPosition = widget.isRTL ? -event.scrollDelta.dx : event.scrollDelta.dx;
      ...
      if (event.scrollDelta.dy.abs() > event.scrollDelta.dx.abs() &&
          viewKey._timelineViewVerticalScrollController!.position.maxScrollExtent == 0) {
        scrolledPosition = widget.isRTL ? -event.scrollDelta.dy : event.scrollDelta.dy;
      }

      final double targetScrollOffset = math.min(
          math.max(viewKey._scrollController!.position.pixels + scrolledPosition,
  ...
  void _updateVisibleDates() {
    ...
    _visibleDates =
        getVisibleDates(currentDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount).cast();
    _previousViewVisibleDates = getVisibleDates(
            widget.isRTL ? nextDate : prevDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount)
        .cast();
    _nextViewVisibleDates = getVisibleDates(
            widget.isRTL ? prevDate : nextDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount)
        .cast();
  ...
  void _updateNextViewVisibleDates() {
    ...
    if (widget.isRTL) {
      currentViewDate = DateTimeHelper.getPreviousViewStartDate(widget.view,
          widget.calendar.monthViewSettings.numberOfWeeksInView, currentViewDate, visibleDatesCount, nonWorkingDays);
    } else {
      currentViewDate = DateTimeHelper.getNextViewStartDate(widget.view,
          widget.calendar.monthViewSettings.numberOfWeeksInView, currentViewDate, visibleDatesCount, nonWorkingDays);
    }

    List<DateTime> dates =
        getVisibleDates(currentViewDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount).cast();
  ...
  List<CalendarTimeRegion> _getRegions(List<DateTime> visibleDates) {
    ...
    for (int j = 0; j < _timeRegions!.length; j++) {
      ...
      region.actualStartTime = AppointmentHelper.convertTimeToAppointmentTimeZone(
    ...
    for (int j = 0; j < recursiveDates.length; j++) {
      ...
      if (region.recurrenceExceptionDates != null) {
        ...
        for (int i = 0; i < region.recurrenceExceptionDates!.length; i++) {
          final DateTime date = AppointmentHelper.convertTimeToAppointmentTimeZone(
  ...
  CalendarTimeRegion cloneRecurrenceRegion(
      CalendarTimeRegion region, DateTime recursiveDate, String? calendarTimeZone) {
    final int minutes = AppointmentHelper.getDifference(region.actualStartTime, region.actualEndTime).inMinutes;
    final DateTime actualEndTime =
        DateTimeHelper.getDateTimeValue(addDuration(recursiveDate, Duration(minutes: minutes)));
    final DateTime startDate =
        AppointmentHelper.convertTimeToAppointmentTimeZone(recursiveDate, calendarTimeZone, region.timeZone);

    final DateTime endDate =
        AppointmentHelper.convertTimeToAppointmentTimeZone(actualEndTime, calendarTimeZone, region.timeZone);
  ...
  void _updateSelectedResourceIndex() {
    for (int i = 0; i < _children.length; i++) {
      ...
      if (selectedResourceIndex != -1) {
        ...
        final int newIndex =
            CalendarViewHelper.getResourceIndex(widget.calendar.dataSource?.resources, selectedResourceId);
  ...
  DateTime _updateSelectedDateForRightArrow(
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if (isSameDate(currentView.visibleDates[visibleDatesCount - 1], selectedDate)) {
        _moveToNextViewWithAnimation();
      }

      selectedDate = AppointmentHelper.addDaysWithTime(
          selectedDate!, 1, selectedDate.hour, selectedDate.minute, selectedDate.second);
      ...
      if (widget.view == CalendarView.month &&
        ...
        for (int i = 0; i < DateTime.daysPerWeek - widget.calendar.timeSlotViewSettings.nonWorkingDays.length; i++) {
          if (widget.calendar.timeSlotViewSettings.nonWorkingDays.contains(selectedDate!.weekday)) {
            selectedDate = AppointmentHelper.addDaysWithTime(
                selectedDate, 1, selectedDate.hour, selectedDate.minute, selectedDate.second);
          } else {
            break;
      ...
      final double xPosition = widget.view == CalendarView.timelineMonth
          ? 0
          : AppointmentHelper.timeToPosition(widget.calendar, selectedDate!, currentViewState._timeIntervalHeight);
      final int rowIndex = _getRowOfDate(currentView.visibleDates, selectedDate!);
      final double singleChildWidth = _getSingleViewWidthForTimeLineView(currentViewState);
      if ((rowIndex * singleChildWidth) + xPosition + currentViewState._timeIntervalHeight >=
          currentViewState._scrollController!.offset + widget.width) {
        currentViewState._scrollController!
            .jumpTo(currentViewState._scrollController!.offset + currentViewState._timeIntervalHeight);
      }
      if (widget.view == CalendarView.timelineDay &&
          selectedDate.add(widget.calendar.timeSlotViewSettings.timeInterval).day !=
              currentView.visibleDates[currentView.visibleDates.length - 1].day) {
        _moveToNextViewWithAnimation();
      }
      ...
      if (widget.view == CalendarView.timelineMonth) {
        ...
        selectedDate = selectedDate.add(widget.calendar.timeSlotViewSettings.timeInterval);
      }
  ...
  DateTime _updateSelectedDateForLeftArrow(
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if ((rowIndex * singleChildWidth) + xPosition <= currentViewState._scrollController!.offset) {
        currentViewState._scrollController!
            .jumpTo(currentViewState._scrollController!.offset - currentViewState._timeIntervalHeight);
      }
      ...
      if (widget.view == CalendarView.timelineMonth) {
        ...
        selectedDate = selectedDate.subtract(widget.calendar.timeSlotViewSettings.timeInterval);
      }
      if (widget.view == CalendarView.timelineWorkWeek) {
        for (int i = 0; i < DateTime.daysPerWeek - widget.calendar.timeSlotViewSettings.nonWorkingDays.length; i++) {
          if (widget.calendar.timeSlotViewSettings.nonWorkingDays.contains(selectedDate!.weekday)) {
  ...
  DateTime? _updateSelectedDateForUpArrow(_CalendarView currentView, _CalendarViewState currentViewState,
      DateTime? selectedDate, DateTime? appointmentSelectedDate) {
    if (widget.view == CalendarView.month) {
      final int rowIndex = _getRowOfDate(currentView.visibleDates, selectedDate!);
      if (rowIndex == 0) {
        return selectedDate;
      ...
      selectedDate = AppointmentHelper.addDaysWithTime(
          selectedDate, -DateTime.daysPerWeek, selectedDate.hour, selectedDate.minute, selectedDate.second);
      ...
      if (!CalendarViewHelper.isCurrentMonthDate(
        ...
        if (CalendarViewHelper.isDateInDateCollection(currentView.blackoutDates, selectedDate)) {
          do {
            selectedDate = AppointmentHelper.addDaysWithTime(
                selectedDate!, 1, selectedDate.hour, selectedDate.minute, selectedDate.second);
          } while (CalendarViewHelper.isDateInDateCollection(currentView.blackoutDates, selectedDate));
        }
      }
      ...
      if (yPosition - 1 <= currentViewState._scrollController!.offset) {
        currentViewState._scrollController!.jumpTo(yPosition - currentViewState._timeIntervalHeight);
      }
      return selectedDate.subtract(widget.calendar.timeSlotViewSettings.timeInterval);
    } else if (CalendarViewHelper.isResourceEnabled(widget.calendar.dataSource, widget.view)) {
      ...
      if (currentViewState._selectedResourceIndex * resourceItemHeight <
        ...
        double scrollPosition = currentViewState._timelineViewVerticalScrollController!.offset - resourceItemHeight;
        scrollPosition = scrollPosition > 0 ? scrollPosition : 0;
        currentViewState._timelineViewVerticalScrollController!.jumpTo(scrollPosition);
      }
  ...
  DateTime? _updateSelectedDateForDownArrow(_CalendarView currentView, _CalendarViewState currentViewState,
      DateTime? selectedDate, DateTime? selectedAppointmentDate) {
    if (widget.view == CalendarView.month) {
      final int rowIndex = _getRowOfDate(currentView.visibleDates, selectedDate!);
      if (rowIndex == widget.calendar.monthViewSettings.numberOfWeeksInView - 1) {
        return selectedDate;
      }
      ...
      if (!CalendarViewHelper.isCurrentMonthDate(
        ...
        selectedDate = AppointmentHelper.getMonthEndDate(
            currentViewState._selectionPainter!.selectedDate ?? selectedAppointmentDate!);
      ...
      if (selectedDate.add(widget.calendar.timeSlotViewSettings.timeInterval).day != selectedDate.day) {
        return selectedDate;
      }

      if (currentViewState._scrollController!.offset + (widget.height - viewHeaderHeight) <
              currentViewState._scrollController!.position.viewportDimension +
                  currentViewState._scrollController!.position.maxScrollExtent &&
          yPosition + currentViewState._timeIntervalHeight + widget.calendar.headerHeight + viewHeaderHeight >=
              currentViewState._scrollController!.offset + widget.height &&
          currentViewState._scrollController!.offset + currentViewState._scrollController!.position.viewportDimension !=
              currentViewState._scrollController!.position.maxScrollExtent) {
        currentViewState._scrollController!
            .jumpTo(currentViewState._scrollController!.offset + currentViewState._timeIntervalHeight);
      }
      return selectedDate.add(widget.calendar.timeSlotViewSettings.timeInterval);
    } else if (CalendarViewHelper.isResourceEnabled(widget.calendar.dataSource, widget.view)) {
      final double resourceItemHeight = CalendarViewHelper.getResourceItemHeight(
          widget.calendar.resourceViewSettings.size,
          widget.height,
          widget.calendar.resourceViewSettings,
          widget.calendar.dataSource!.resources!.length);
      if (currentViewState._selectedResourceIndex == widget.calendar.dataSource!.resources!.length - 1 ||
          currentViewState._selectedResourceIndex == -1) {
        return selectedDate;
      }
      ...
      if (currentViewState._selectedResourceIndex * resourceItemHeight >=
          currentViewState._timelineViewVerticalScrollController!.offset +
              currentViewState._timelineViewVerticalScrollController!.position.viewportDimension) {
        double scrollPosition = currentViewState._timelineViewVerticalScrollController!.offset + resourceItemHeight;
        scrollPosition =
            scrollPosition > currentViewState._timelineViewVerticalScrollController!.position.maxScrollExtent
                ? currentViewState._timelineViewVerticalScrollController!.position.maxScrollExtent
                : scrollPosition;
        currentViewState._timelineViewVerticalScrollController!.jumpTo(scrollPosition);
      }
  ...
  void _moveToSelectedTimeSlot() {
    ...
    final double scrollPosition =
        currentViewState._getScrollPositionForCurrentDate(currentViewState._selectionPainter!.selectedDate!);
    if (scrollPosition == -1 || currentViewState._scrollController!.position.pixels == scrollPosition) {
      return;
    }

    currentViewState._scrollController!.jumpTo(
        currentViewState._scrollController!.position.maxScrollExtent > scrollPosition
            ? scrollPosition
            : currentViewState._scrollController!.position.maxScrollExtent);
  }

  DateTime? _updateSelectedDate(RawKeyEvent event, _CalendarViewState currentViewState, _CalendarView currentView,
      int resourceIndex, DateTime? selectedAppointmentDate, bool isAllDayAppointment) {
    DateTime? selectedDate = currentViewState._selectionPainter!.selectedDate ?? selectedAppointmentDate;
    if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
      do {
        selectedDate = _updateSelectedDateForRightArrow(currentView, currentViewState, selectedDate);
      } while (!_isSelectedDateEnabled(selectedDate, resourceIndex, true));

      return selectedDate;
    } else if (event.logicalKey == LogicalKeyboardKey.arrowLeft) {
      do {
        selectedDate = _updateSelectedDateForLeftArrow(currentView, currentViewState, selectedDate);
      } while (!_isSelectedDateEnabled(selectedDate, resourceIndex, true));
      ...
      do {
        selectedDate =
            _updateSelectedDateForUpArrow(currentView, currentViewState, selectedDate, selectedAppointmentDate);
        if (resourceIndex != -1 && currentView.regions != null && currentView.regions!.isNotEmpty) {
          resourceIndex -= 1;
        }

        if (widget.controller.view != CalendarView.month && !CalendarViewHelper.isTimelineView(widget.calendar.view)) {
          double yPosition =
              AppointmentHelper.timeToPosition(widget.calendar, selectedDate!, currentViewState._timeIntervalHeight);
          if (yPosition < 1 && !_isSelectedDateEnabled(selectedDate, resourceIndex, true)) {
            yPosition = AppointmentHelper.timeToPosition(widget.calendar,
                currentViewState._selectionPainter!.selectedDate!, currentViewState._timeIntervalHeight);
      ...
      do {
        selectedDate =
            _updateSelectedDateForDownArrow(currentView, currentViewState, selectedDate, selectedAppointmentDate);
        if (resourceIndex != -1 && currentView.regions != null && currentView.regions!.isNotEmpty) {
          resourceIndex += 1;
        }
  ...
  bool _isSelectedDateEnabled(DateTime date, int resourceIndex, [bool isMinMaxDate = false]) {
    final bool isMonthView = widget.view == CalendarView.month || widget.view == CalendarView.timelineMonth;
    final int timeInterval = CalendarViewHelper.getTimeInterval(widget.calendar.timeSlotViewSettings);
    if ((isMonthView && !isDateWithInDateRange(widget.calendar.minDate, widget.calendar.maxDate, date)) ||
        (!isMonthView &&
            !CalendarViewHelper.isDateTimeWithInDateTimeRange(
                widget.calendar.minDate, widget.calendar.maxDate, date, timeInterval))) {
      return isMinMaxDate;
    }

    if (isMonthView && CalendarViewHelper.isDateInDateCollection(_getBlackoutDates(), date)) {
      return false;
    } else if (!isMonthView) {
      ...
      for (int i = 0; i < regions.length; i++) {
        ...
        if (region.enablePointerInteraction ||
            (region.actualStartTime.isAfter(date) &&
                !CalendarViewHelper.isSameTimeSlot(region.actualStartTime, date)) ||
  ...
  KeyEventResult _updatePageUpAndDown(RawKeyEvent event, _CalendarViewState currentViewState, bool isResourceEnabled) {
    ...
    final ScrollController scrollController =
        isResourceEnabled ? widget.resourcePanelScrollController! : currentViewState._scrollController!;
    ...
    if (event.logicalKey == LogicalKeyboardKey.pageUp ||
        (platform == TargetPlatform.windows && event.logicalKey.keyId == 0x10700000021)) {
      ...
      return KeyEventResult.handled;
        ...
        (platform == TargetPlatform.windows && event.logicalKey.keyId == 0x10700000022)) {
      double viewHeaderHeight =
          CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.controller.view!);
      ...
      final double timeRulerSize = CalendarViewHelper.getTimeLabelWidth(
          widget.calendar.timeSlotViewSettings.timeRulerSize, widget.controller.view!);
      ...
      final double viewPortEndPosition = scrollController.position.pixels + viewPortHeight;
      if (viewPortEndPosition == scrollViewHeight) {
        return KeyEventResult.ignored;
      }
      difference = (scrollViewHeight - scrollController.position.pixels) * divideValue;
  ...
  KeyEventResult _updateAppointmentSelection(RawKeyEvent event, _CalendarViewState currentVisibleViewState,
      bool isResourceEnabled, AppointmentView? currentSelectedAppointment, AppointmentView? currentAllDayAppointment) {
    ...
    final List<AppointmentView> appointmentCollection =
        currentVisibleViewState._appointmentLayout.getAppointmentViewCollection();
    final List<AppointmentView> allDayAppointmentCollection =
        _updateCalendarStateDetails.allDayAppointmentViewCollection;
    final List<AppointmentView> tempAppColl = isAllDay ? allDayAppointmentCollection : appointmentCollection;
    if (event.isShiftPressed) {
      if (event.logicalKey == LogicalKeyboardKey.tab) {
        if (currentAllDayAppointment != null || currentSelectedAppointment != null) {
          int index = tempAppColl.indexOf(isAllDay ? currentAllDayAppointment : currentSelectedAppointment!);
          index -= 1;
          if (tempAppColl.length > index && !index.isNegative) {
            selectedAppointment = tempAppColl[index].appointment != null ? tempAppColl[index] : null;
          }
        }

        if (currentSelectedAppointment != null && selectedAppointment == null) {
          isAllDay = allDayAppointmentCollection.isNotEmpty;
          selectedAppointment = isAllDay ? allDayAppointmentCollection[allDayAppointmentCollection.length - 1] : null;
        } else if (currentSelectedAppointment == null &&
            currentAllDayAppointment == null &&
            selectedAppointment == null) {
          if (currentVisibleViewState._selectionPainter!.selectedDate != null && appointmentCollection.isNotEmpty) {
            for (int i = 0; i < appointmentCollection.length; i++) {
              if (AppointmentHelper.getDifference(currentVisibleViewState._selectionPainter!.selectedDate!,
            ...
            selectedAppointment =
                appointmentCollection.isNotEmpty ? appointmentCollection[appointmentCollection.length - 1] : null;
          }
        }

        return _updateAppointmentSelectionOnView(
            selectedAppointment, currentVisibleViewState, isAllDay, isResourceEnabled, !event.isShiftPressed);
      }
      ...
      } else if (currentAllDayAppointment == null && currentSelectedAppointment == null) {
        if (currentVisibleViewState._selectionPainter!.selectedDate != null && appointmentCollection.isNotEmpty) {
          for (int i = 0; i < appointmentCollection.length; i++) {
            if (AppointmentHelper.getDifference(currentVisibleViewState._selectionPainter!.selectedDate!,
                    appointmentCollection[i].appointment!.actualStartTime)
  ...
  /// in Calendar.
  KeyEventResult _updateAppointmentSelectionOnView(AppointmentView? selectedAppointment,
      _CalendarViewState currentVisibleViewState, bool isAllDay, bool isResourceEnabled, bool isForward) {
    final DateTime visibleStartDate =
        AppointmentHelper.convertToStartTime(currentVisibleViewState.widget.visibleDates[0]);
    final DateTime visibleEndDate = AppointmentHelper.convertToEndTime(
        currentVisibleViewState.widget.visibleDates[currentVisibleViewState.widget.visibleDates.length - 1]);
    ...
    if (isAllDay && selectedAppointment != null) {
      ...
      currentVisibleViewState._selectionNotifier.value = !currentVisibleViewState._selectionNotifier.value;
    ...
    if (selectedAppointment != null &&
        AppointmentHelper.isAppointmentWithinVisibleDateRange(
            selectedAppointment.appointment!, visibleStartDate, visibleEndDate)) {
      currentVisibleViewState._allDaySelectionNotifier.value = null;
      currentVisibleViewState._selectionPainter!.appointmentView = selectedAppointment;
      currentVisibleViewState._selectionPainter!.selectedDate = null;
      currentVisibleViewState._selectionNotifier.value = !currentVisibleViewState._selectionNotifier.value;
      ...
      if (widget.controller.view != CalendarView.month) {
        ...
        final double scrollViewHeight = currentVisibleViewState._scrollController!.position.maxScrollExtent +
            currentVisibleViewState._scrollController!.position.viewportDimension;
        final double resourceViewSize = isResourceEnabled ? widget.calendar.resourceViewSettings.size : 0;
        final bool isTimeline = CalendarViewHelper.isTimelineView(widget.controller.view!);
        double viewHeaderHeight =
            CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.controller.view!);
        ...
        _updateScrollViewToAppointment(
            offset, currentVisibleViewState._scrollController!, viewPortSize, scrollViewHeight);

        if (isResourceEnabled) {
          final double resourcePanelHeight = widget.resourcePanelScrollController!.position.viewportDimension +
              widget.resourcePanelScrollController!.position.maxScrollExtent;
          final double timeRulerSize = CalendarViewHelper.getTimeLabelWidth(
                  widget.calendar.timeSlotViewSettings.timeRulerSize, widget.controller.view!),
              viewPortSize = widget.height - viewHeaderHeight - timeRulerSize;
          _updateScrollViewToAppointment(selectedAppointment.appointmentRect!.top,
              widget.resourcePanelScrollController!, viewPortSize, resourcePanelHeight);
        }
      ...
      _updateCalendarStateDetails.selectedDate = null;
      widget.updateCalendarState(_updateCalendarStateDetails);
      isForward ? FocusScope.of(context).nextFocus() : FocusScope.of(context).previousFocus();
      return KeyEventResult.handled;
    }
  }
  ...
  void _updateScrollViewToAppointment(
      double offset, ScrollController scrollController, double viewPortSize, double panelHeight) {
    if (offset < scrollController.position.pixels || offset > (scrollController.position.pixels + viewPortSize)) {
  ...
  KeyEventResult _onKeyDown(FocusNode node, RawKeyEvent event) {
    ...
    CalendarViewHelper.handleViewSwitchKeyBoardEvent(event, widget.controller, widget.calendar.allowedViews);
    ...
    result = _updatePageUpAndDown(event, currentVisibleViewState, isResourcesEnabled);

    AppointmentView? currentSelectedAppointment = currentVisibleViewState._selectionPainter!.appointmentView;
    AppointmentView? currentAllDayAppointment = currentVisibleViewState._allDaySelectionNotifier.value?.appointmentView;

    result = _updateAppointmentSelection(
        event, currentVisibleViewState, isResourcesEnabled, currentSelectedAppointment, currentAllDayAppointment);

    currentSelectedAppointment = currentVisibleViewState._selectionPainter!.appointmentView;
    currentAllDayAppointment = currentVisibleViewState._allDaySelectionNotifier.value?.appointmentView;

    if (event.logicalKey == LogicalKeyboardKey.enter &&
        CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.calendar.onTap)) {
      ...
      final List<CalendarAppointment>? selectedAppointments =
          widget.controller.view == CalendarView.month && selectedAppointment == null
              ? AppointmentHelper.getSelectedDateAppointments(_updateCalendarStateDetails.appointments,
                  widget.calendar.timeZone, _updateCalendarStateDetails.selectedDate)
              : selectedAppointment != null
                  ? <CalendarAppointment>[selectedAppointment.appointment!]
                  : null;
      final CalendarElement tappedElement =
          _updateCalendarStateDetails.selectedDate != null ? CalendarElement.calendarCell : CalendarElement.appointment;
      ...
      CalendarViewHelper.raiseCalendarTapCallback(
          ...
          CalendarViewHelper.getCustomAppointments(selectedAppointments, widget.calendar.dataSource),
          tappedElement,
          isResourcesEnabled
              ? widget.calendar.dataSource!.resources![currentVisibleViewState._selectedResourceIndex]
              : null);
    }

    final int previousResourceIndex = isResourcesEnabled ? currentVisibleViewState._selectedResourceIndex : -1;
    ...
    if ((currentVisibleViewState._selectionPainter!.selectedDate != null &&
        ...
        (currentSelectedAppointment != null || currentAllDayAppointment != null)) {
      final int resourceIndex = isResourcesEnabled ? currentVisibleViewState._selectedResourceIndex : -1;

      final DateTime? selectedAppointmentDate = currentAllDayAppointment != null
          ? AppointmentHelper.convertToStartTime(currentAllDayAppointment.appointment!.actualStartTime)
          : currentSelectedAppointment?.appointment!.actualStartTime;

      final bool isAllDayAppointment = currentAllDayAppointment != null;

      final DateTime? selectedDate = _updateSelectedDate(event, currentVisibleViewState, currentVisibleView,
          resourceIndex, selectedAppointmentDate, isAllDayAppointment);
      ...
      if (widget.calendar.onSelectionChanged != null &&
          (!CalendarViewHelper.isSameTimeSlot(currentVisibleViewState._selectionPainter!.selectedDate, selectedDate) ||
              (isResourcesEnabled &&
                  currentVisibleViewState._selectionPainter!.selectedResourceIndex !=
                      currentVisibleViewState._selectedResourceIndex))) {
        CalendarViewHelper.raiseCalendarSelectionChangedCallback(widget.calendar, selectedDate,
            isResourcesEnabled ? widget.resourceCollection![currentVisibleViewState._selectedResourceIndex] : null);
      }
      ...
      currentVisibleViewState._selectionPainter!.appointmentView = null;
      currentVisibleViewState._selectionPainter!.selectedResourceIndex = currentVisibleViewState._selectedResourceIndex;
      currentVisibleViewState._selectionNotifier.value = !currentVisibleViewState._selectionNotifier.value;
  ...
  void _positionTimelineView({bool isScrolledToEnd = true}) {
    ...
    if (widget.isRTL) {
      if (_currentChildIndex == 0) {
        ...
        nextViewState._scrollController!
            .jumpTo(isScrolledToEnd ? nextViewState._scrollController!.position.maxScrollExtent : 0);
  ...
  void _onHorizontalStart(DragStartDetails dragStartDetails, bool isResourceEnabled, bool isTimelineView,
      double viewHeaderHeight, double timeLabelWidth, bool isNeedDragAndDrop) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
  ...
  void _onHorizontalUpdate(DragUpdateDetails dragUpdateDetails,
    ...
    if (_dragDetails.value.appointmentView != null && !widget.isMobilePlatform && isNeedDragAndDrop) {
      _handleLongPressMove(
          Offset(dragUpdateDetails.localPosition.dx - widget.width, dragUpdateDetails.localPosition.dy),
          isTimelineView,
          isResourceEnabled,
          isMonthView,
          viewHeaderHeight,
  ...
  void _onVerticalStart(DragStartDetails dragStartDetails, bool isResourceEnabled, bool isTimelineView,
      double viewHeaderHeight, double timeLabelWidth, bool isNeedDragAndDrop) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
    if (currentState._hoveringAppointmentView != null &&
        currentState._hoveringAppointmentView!.appointment != null &&
      ...
      _handleAppointmentDragStart(
          currentState._hoveringAppointmentView!.clone(),
          isTimelineView,
          Offset(dragStartDetails.localPosition.dx, dragStartDetails.localPosition.dy - widget.height),
          isResourceEnabled,
          viewHeaderHeight,
          timeLabelWidth);
      return;
  ...
  void _onVerticalUpdate(DragUpdateDetails dragUpdateDetails,
    ...
    if (_dragDetails.value.appointmentView != null && !widget.isMobilePlatform && isNeedDragAndDrop) {
      _handleLongPressMove(
          Offset(dragUpdateDetails.localPosition.dx, dragUpdateDetails.localPosition.dy - widget.height),
          isTimelineView,
          isResourceEnabled,
          isMonthView,
          viewHeaderHeight,
...
class _CalendarViewState extends State<_CalendarView> with TickerProviderStateMixin {
  ...
  void initState() {
    _resizingDetails =
        ValueNotifier<_ResizingPaintDetails>(_ResizingPaintDetails(position: ValueNotifier<Offset?>(null)));
    ...
    _timeIntervalHeight = _getTimeIntervalHeight(
        widget.calendar, widget.view, widget.width, widget.height, widget.visibleDates.length, widget.isMobilePlatform);
    if (widget.view != CalendarView.month) {
  ...
  Widget _getMonthView() {
    final SystemMouseCursor currentCursor =
        _mouseCursor == SystemMouseCursors.resizeUp || _mouseCursor == SystemMouseCursors.resizeDown
  ...
  Widget _getDayView() {
    ...
    if (isCurrentView && _updateCalendarStateDetails.allDayPanelHeight != 0) {
      ...
      if (_allDayHeight == defaultAllDayHeight) {
        _timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view, widget.width, widget.height,
            widget.visibleDates.length, widget.isMobilePlatform);
      }
    }
        ...
        children: <Widget>[
          GestureDetector(
            ...
            child: SizedBox(
                ...
                width: widget.width,
                child: _addDayView(
                    widget.width, _timeIntervalHeight * _horizontalLinesCount!, _isRTL, widget.locale, isCurrentView)),
          ),
          _getResizeShadowView()
        ],
      ),
        ...
        child: Stack(children: <Widget>[
          GestureDetector(
            ...
            child: SizedBox(
              width: widget.width,
              height: widget.height,
              child: _addTimelineView(_timeIntervalHeight * (_horizontalLinesCount! * widget.visibleDates.length),
                  widget.height, widget.locale),
            ),
          ),
          _getResizeShadowView()
        ]));
  ...
  void _retainScrolledDateTime() {
    ...
    final double time =
        ((CalendarViewHelper.getTimeInterval(widget.calendar.timeSlotViewSettings) / 60) * columnIndex) +
  ...
  double _getPositionFromDate(DateTime date) {
    final int visibleDatesCount = widget.visibleDates.length;
    _timeIntervalHeight = _getTimeIntervalHeight(
        widget.calendar, widget.view, widget.width, widget.height, visibleDatesCount, widget.isMobilePlatform);
    double timeToPosition = 0;
    final bool isTimelineView = CalendarViewHelper.isTimelineView(widget.view);
    if (!isTimelineView) {
      timeToPosition = AppointmentHelper.timeToPosition(widget.calendar, date, _timeIntervalHeight);
  ...
  Widget _addAllDayAppointmentPanel(SfCalendarThemeData calendarTheme, bool isCurrentView) {
    ...
    if (_allDayHeight == 0 ||
        (!isDayView && widget.visibleDates != _updateCalendarStateDetails.currentViewVisibleDates)) {
        ...
        children: <Widget>[
          Positioned(
            ...
            child: ListView(
              ...
              children: <Widget>[_getAllDayLayout(timeLabelWidth, panelHeight, allDayExpanderHeight, isCurrentView)],
            ),
          ),
  ...
  Widget _getAllDayLayout(double timeLabelWidth, double panelHeight, double allDayExpanderHeight, bool isCurrentView) {
    final bool isDayView = CalendarViewHelper.isDayView(
        widget.view,
        widget.calendar.timeSlotViewSettings.numberOfDaysInView,
        widget.calendar.timeSlotViewSettings.nonWorkingDays,
    ...
    final Widget allDayLayout = AllDayAppointmentLayout(
        ...
        widget.isMobilePlatform,
        widget.width,
        (isDayView && _updateCalendarStateDetails.allDayPanelHeight < _allDayHeight) || !isCurrentView
            ? _allDayHeight
            : _updateCalendarStateDetails.allDayPanelHeight,
        widget.localizations,
  ...
  void _onVerticalStart(DragStartDetails details) {
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view) && widget.view != CalendarView.month) {
      ...
      _updateMaximumResizingPosition(
          isForwardResize, isBackwardResize, appointmentView, allDayPanelHeight, viewHeaderHeight);
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeStartCallback(widget.calendar.onAppointmentResizeStart)) {
      CalendarViewHelper.raiseAppointmentResizeStartCallback(
          widget.calendar, _getCalendarAppointmentToObject(appointmentView!.appointment, widget.calendar), null);
    }
  }
  ...
  void _onVerticalUpdate(DragUpdateDetails details) {
    ...
    final double allDayPanelHeight = _isExpanded ? _updateCalendarStateDetails.allDayPanelHeight : _allDayHeight;
    if (!CalendarViewHelper.isTimelineView(widget.view) && widget.view != CalendarView.month) {
      _updateMaximumResizingPosition(isForwardResize, isBackwardResize, _resizingDetails.value.appointmentView!,
    ...
    _resizingDetails.value.position.value =
        Offset(_resizingDetails.value.appointmentView!.appointmentRect!.left, yPosition);
  ...
  void _onVerticalEnd(DragEndDetails details) {
    ...
    final double currentYPosition = _resizingDetails.value.position.value!.dy > widget.height - 1
        ? widget.height - 1
    ...
    final DateTime resizingTime = _timeFromPosition(
        appointment.actualStartTime, widget.calendar.timeSlotViewSettings, yPosition, null, timeIntervalHeight, false)!;
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
      ...
      if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(widget.calendar.onAppointmentResizeEnd)) {
        CalendarViewHelper.raiseAppointmentResizeEndCallback(
            widget.calendar, appointment.data, null, appointment.exactStartTime, appointment.exactEndTime);
      }
    ...
    if (appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) {
      ...
      final DateTime exceptionDate =
          AppointmentHelper.convertTimeToAppointmentTimeZone(appointment.exactStartTime, widget.calendar.timeZone, '');
      parentAppointment.recurrenceExceptionDates != null
          ? parentAppointment.recurrenceExceptionDates!.add(exceptionDate)
          : parentAppointment.recurrenceExceptionDates = <DateTime>[exceptionDate];
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(widget.calendar.onAppointmentResizeEnd)) {
      CalendarViewHelper.raiseAppointmentResizeEndCallback(
          widget.calendar, newAppointment, null, callbackStartDate, callbackEndDate);
    }
  ...
  void _onHorizontalStart(DragStartDetails details) {
    ...
    if (!isTimelineView && widget.view != CalendarView.month) {
      ...
      appointmentView = _getAllDayAppointmentOnPoint(
          _updateCalendarStateDetails.allDayAppointmentViewCollection, xPosition, yPosition);
      if (appointmentView == null) {
        return;
      }
      ...
      if (_isRTL) {
        xPosition =
            _scrollController!.offset + (_scrollController!.position.viewportDimension - details.localPosition.dx);
        xPosition =
            (_scrollController!.position.viewportDimension + _scrollController!.position.maxScrollExtent) - xPosition;
      }
      ...
      if (isResourceEnabled) {
        resource = widget.calendar.dataSource!.resources![
            _getSelectedResourceIndex(appointmentView.appointmentRect!.top, viewHeaderHeight, timeLabelWidth)];
      }
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeStartCallback(widget.calendar.onAppointmentResizeStart)) {
      CalendarViewHelper.raiseAppointmentResizeStartCallback(
          widget.calendar, _getCalendarAppointmentToObject(appointmentView.appointment, widget.calendar), resource);
    }
  }
  ...
  void _onHorizontalUpdate(DragUpdateDetails details) {
    ...
    if (isTimelineView) {
      _updateMaximumResizingPosition(
          isForwardResize, isBackwardResize, _resizingDetails.value.appointmentView!, null, null);
      ...
      _updateAutoScrollTimeline(details, timeIntervalHeight, isForwardResize, isBackwardResize, xPosition, yPosition,
          timeLabelWidth, isResourceEnabled);
    } else if (widget.view == CalendarView.month) {
      final double viewHeaderHeight =
          CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view);
      ...
      final double cellHeight =
          (widget.height - viewHeaderHeight) / widget.calendar.monthViewSettings.numberOfWeeksInView;
      final int appointmentRowIndex =
          (_resizingDetails.value.appointmentView!.appointmentRect!.top / cellHeight).truncate();
      ...
      if (!widget.calendar.monthViewSettings.showTrailingAndLeadingDates &&
        ...
        final int startIndex = DateTimeHelper.getVisibleDateIndex(
            widget.visibleDates, AppointmentHelper.getMonthStartDate(currentMonthDate));
        final int endIndex = DateTimeHelper.getVisibleDateIndex(
            widget.visibleDates, AppointmentHelper.getMonthEndDate(currentMonthDate));
      ...
      if (_isRTL) {
        if (isForwardResize &&
            ((appointmentRowIndex == resizingRowIndex && xPosition < _maximumResizingPosition!) ||
      ...
      final int rowDifference = isBackwardResize
          ...
          : _isRTL
              ...
              : (appointmentRowIndex - resizingRowIndex).abs();
      if (((!_isRTL &&
                  ((isBackwardResize && appointmentRowIndex > resizingRowIndex) ||
                      (isForwardResize && appointmentRowIndex < resizingRowIndex))) ||
              (_isRTL &&
                  ((isBackwardResize && appointmentRowIndex < resizingRowIndex) ||
                      (isForwardResize && appointmentRowIndex > resizingRowIndex)))) &&
          resizingRowIndex != appointmentRowIndex &&
    ...
    if (isTimelineView) {
      _updateAppointmentResizingUpdateCallback(isForwardResize, isBackwardResize, yPosition, null, null,
          xPosition: xPosition, timeLabelWidth: timeLabelWidth, isResourceEnabled: isResourceEnabled, details: details);
      return;
    }
  ...
  void _onHorizontalEnd(DragEndDetails details) {
    ...
    if (_resizingDetails.value.isAllDayPanel ||
      ...
      final DateTime time = _timeFromPosition(
          resizingTime, widget.calendar.timeSlotViewSettings, xPosition, this, timeIntervalHeight, isTimelineView)!;

      resizingTime =
          DateTime(resizingTime.year, resizingTime.month, resizingTime.day, time.hour, time.minute, time.second);
    }
    ...
    if (isResourceEnabled) {
      selectedResourceIndex = _getSelectedResourceIndex(
          _resizingDetails.value.appointmentView!.appointmentRect!.top, viewHeaderHeight, timeLabelWidth);
    ...
    if ((_isRTL && _mouseCursor == SystemMouseCursors.resizeLeft) ||
      ...
      if (isMonthView) {
        updatedEndTime = DateTime(
            resizingTime.year, resizingTime.month, resizingTime.day, updatedEndTime.hour, updatedEndTime.minute);
      } else {
        updatedEndTime = resizingTime;
      }
      ...
      if (isMonthView) {
        updatedStartTime = DateTime(
            resizingTime.year, resizingTime.month, resizingTime.day, updatedStartTime.hour, updatedStartTime.minute);
      } else {
        updatedStartTime = resizingTime;
      }
    }
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
      ...
      if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(widget.calendar.onAppointmentResizeEnd)) {
        CalendarViewHelper.raiseAppointmentResizeEndCallback(
            widget.calendar, appointment.data, resource, appointment.exactStartTime, appointment.exactEndTime);
      }
    ...
    if ((appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) ||
      ...
      final List<DateTime> recurrenceDates = RecurrenceHelper.getRecurrenceDateTimeCollection(
          ...
          specificEndDate: widget.visibleDates[widget.visibleDates.length - 1]);
      ...
      if (currentRecurrenceIndex != -1) {
        final DateTime? previousRecurrence =
            currentRecurrenceIndex <= 0 ? null : recurrenceDates[currentRecurrenceIndex - 1];
        final DateTime? nextRecurrence =
            currentRecurrenceIndex >= recurrenceDates.length - 1 ? null : recurrenceDates[currentRecurrenceIndex + 1];
        ...
        /// occurrence.
        if (!((nextRecurrence == null || isSameOrBeforeDate(nextRecurrence, resizingTime)) &&
                (previousRecurrence == null || isSameOrAfterDate(previousRecurrence, resizingTime)) &&
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(widget.calendar.onAppointmentResizeEnd)) {
      CalendarViewHelper.raiseAppointmentResizeEndCallback(
          widget.calendar, newAppointment, resource, callbackStartDate, callbackEndDate);
    }

    _resetResizingPainter();
  }

  Future<void> _updateAutoScrollDay(DragUpdateDetails details, double viewHeaderHeight, double allDayPanelHeight,
      bool isForwardResize, bool isBackwardResize, double? yPosition) async {
    if (_resizingDetails.value.appointmentView == null) {
      return;
    }
      ...
      _autoScrollTimer = Timer(const Duration(milliseconds: 200), () async {
        ...
        if (yPosition != null && yPosition! <= viewHeaderHeight + allDayPanelHeight && _scrollController!.offset != 0) {
          Future<void> updateScrollPosition() async {
            ...
            _resizingDetails.value.position.value =
                Offset(_resizingDetails.value.appointmentView!.appointmentRect!.left, yPosition! - 0.1);
  ...
  Future<void> _updateAutoScrollTimeline(DragUpdateDetails details, double timeIntervalHeight, bool isForwardResize,
      bool isBackwardResize, double? xPosition, double yPosition, double timeLabelWidth, bool isResourceEnabled) async {
    if (_resizingDetails.value.appointmentView == null) {
      return;
    }
    ...
    if (xPosition! <= 0 &&
        ((_isRTL && _scrollController!.position.pixels != _scrollController!.position.maxScrollExtent) ||
  ...
  void _updateMaximumResizingPosition(bool isForwardResize, bool isBackwardResize, AppointmentView appointmentView,
      double? allDayPanelHeight, double? viewHeaderHeight) {
    switch (widget.view) {
      case CalendarView.schedule:
        break;
      case CalendarView.day:
      ...
      case CalendarView.workWeek:
        {
          if (_resizingDetails.value.isAllDayPanel) {
            final double timeLabelWidth =
                CalendarViewHelper.getTimeLabelWidth(widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
            final double minimumCellWidth = ((widget.width - timeLabelWidth) / widget.visibleDates.length) / 2;
            if (isForwardResize) {
            ...
            if (isForwardResize) {
              _maximumResizingPosition = (appointmentView.appointmentRect!.top -
                      ...
                      allDayPanelHeight! +
                      viewHeaderHeight!) +
                  (appointmentView.appointmentRect!.height / 2 > minimumTimeIntervalSize
                      ? minimumTimeIntervalSize
                      : appointmentView.appointmentRect!.height / 2);
            } else if (isBackwardResize) {
              _maximumResizingPosition = (appointmentView.appointmentRect!.bottom -
                      _scrollController!.offset +
                      allDayPanelHeight! +
                      viewHeaderHeight!) -
                  (appointmentView.appointmentRect!.height / 2 > minimumTimeIntervalSize
      ...
      case CalendarView.timelineMonth:
        {
          ...
          if (isForwardResize) {
            ...
            _maximumResizingPosition = _maximumResizingPosition! +
                (appointmentView.appointmentRect!.width / 2 > minimumTimeIntervalSize
                    ? minimumTimeIntervalSize
  ...
  void _updateAppointmentResizingUpdateCallback(bool isForwardResize, bool isBackwardResize, double yPosition,
      double? viewHeaderHeight, double? allDayPanelHeight,
      {bool isResourceEnabled = false, double? timeLabelWidth, double? xPosition, DragUpdateDetails? details}) {
    final double timeIntervalHeight = _getTimeIntervalHeight(
        widget.calendar, widget.view, widget.width, widget.height, widget.visibleDates.length, widget.isMobilePlatform);
    late DateTime resizingTime;
    CalendarResource? resource;
  ...
  Widget _getResizeShadowView() {
    ...
    final bool isVerticalResize =
        _mouseCursor == SystemMouseCursors.resizeUp || _mouseCursor == SystemMouseCursors.resizeDown;
    final bool isTimelineView = CalendarViewHelper.isTimelineView(widget.view);
    final bool isAllDayPanel = !isVerticalResize && (!isTimelineView && widget.view != CalendarView.month);
    final double timeLabelWidth =
        CalendarViewHelper.getTimeLabelWidth(widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    final double weekNumberPanelWidth = CalendarViewHelper.getWeekNumberPanelWidth(
        widget.calendar.showWeekNumber, widget.width, widget.isMobilePlatform);
    ...
    final double overAllWidth =
        isTimelineView ? _timeIntervalHeight * (_horizontalLinesCount! * widget.visibleDates.length) : widget.width;
    ...
    return Positioned(
        ...
        child: GestureDetector(
          ...
          child: IgnorePointer(
              ignoring:
                  _mouseCursor == SystemMouseCursors.basic || _mouseCursor == SystemMouseCursors.move || isAllDayPanel,
              child: RepaintBoundary(
                  child: CustomPaint(
  ...
  Widget _addDayView(double width, double height, bool isRTL, String locale, bool isCurrentView) {
  ...
  void _updateProgrammaticSelectedResourceIndex() {
    if (_updateCalendarStateDetails.selectedDate != null && _selectedResourceIndex == -1) {
      ...
      if ((isTimelineMonth &&
              (isSameDate(_updateCalendarStateDetails.selectedDate, widget.calendar.initialSelectedDate))) ||
          (!isTimelineMonth &&
              (CalendarViewHelper.isSameTimeSlot(
  ...
  CalendarDetails? _getDetailsForMonth(Offset position) {
    ...
    if (yPosition < viewHeaderHeight) {
      ...
      return CalendarDetails(null, _getTappedViewHeaderDate(position, widget.width), CalendarElement.viewHeader, null);
    }
    ...
    if (appointmentView == null) {
      ...
      if (!CalendarViewHelper.isCurrentMonthDate(widget.calendar.monthViewSettings.numberOfWeeksInView,
          widget.calendar.monthViewSettings.showTrailingAndLeadingDates, currentMonth, getDate)) {
    ...
    return CalendarDetails(selectedAppointments, getDate, selectedElement, null);
  }
  ...
  AppointmentView? _handleTouchOnMonthView(TapUpDetails? tapDetails, LongPressStartDetails? longPressDetails) {
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      final bool canRaiseTap =
          CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.calendar.onTap) && isTapCallback;
  ...
  void _updatedSelectionChangedCallback(bool canRaiseSelectionChanged, DateTime? previousSelectedDate,
    ...
    if (canRaiseSelectionChanged &&
        ((isMonthView && !isSameDate(previousSelectedDate, _selectionPainter!.selectedDate)) ||
            (!isMonthView &&
                !CalendarViewHelper.isSameTimeSlot(previousSelectedDate, _selectionPainter!.selectedDate)) ||
            (CalendarViewHelper.isResourceEnabled(widget.calendar.dataSource, widget.view) &&
  ...
  CalendarDetails? _getDetailsForTimeline(Offset position) {
    ...
    if (appointmentView == null) {
      ...
      return CalendarDetails(null, getDate, CalendarElement.calendarCell, calendarResource);
    } else {
      /// Return calendar details while the [getCalendarDetailsAtOffset]
  ...
  CalendarDetails? _getDetailsForDay(Offset position) {
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (isDayView &&
        ...
        return CalendarDetails(
            null, _getTappedViewHeaderDate(position, widget.width), CalendarElement.viewHeader, null);
      ...
      final AppointmentView? appointmentView = _getAllDayAppointmentOnPoint(
          _updateCalendarStateDetails.allDayAppointmentViewCollection, xDetails, yPosition);
      ...
      if (appointmentView != null &&
        ...
        return CalendarDetails(null, selectedDate, CalendarElement.allDayPanel, null);
      }
    ...
    if (appointmentView == null) {
      ...
      final DateTime? selectedDate =
          _getDateFromPosition(!_isRTL ? xDetails - timeLabelWidth : xDetails, yPosition, timeLabelWidth);

      /// Return calendar details while the [getCalendarDetailsAtOffset]
      /// position placed on calendar cell in day, week and work week view.
      return CalendarDetails(null, selectedDate, CalendarElement.calendarCell, null);
    } else {
      final List<dynamic> appointmentDetails = <dynamic>[
        CalendarViewHelper.getAppointmentDetail(appointmentView.appointment!, widget.calendar.dataSource)
      ];
  ...
  AppointmentView? _handleTouchOnDayView(TapUpDetails? tapDetails, LongPressStartDetails? longPressDetails) {
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (appointmentView != null &&
          (yPosition < allDayHeight - kAllDayAppointmentHeight ||
              _updateCalendarStateDetails.allDayPanelHeight <= allDayHeight ||
              appointmentView.position + 1 >= appointmentView.maxPositions)) {
        if (!CalendarViewHelper.isDateTimeWithInDateTimeRange(widget.calendar.minDate, widget.calendar.maxDate,
                appointmentView.appointment!.actualStartTime, timeInterval) ||
            !CalendarViewHelper.isDateTimeWithInDateTimeRange(widget.calendar.minDate, widget.calendar.maxDate,
                appointmentView.appointment!.actualEndTime, timeInterval)) {
          return null;
        }
        if (selectedDate != null) {
          selectedDate = null;
      ...
      if (appointmentView == null) {
        if (_isRTL) {
          _drawSelection(xDetails, yDetails - viewHeaderHeight - allDayHeight, timeLabelWidth);
        } else {
    ...
    widget.updateCalendarState(_updateCalendarStateDetails);
    final bool canRaiseTap =
        CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.calendar.onTap) && isTappedCallback;
    final bool canRaiseLongPress =
    ...
    if (canRaiseLongPress || canRaiseTap || canRaiseSelectionChanged) {
      ...
      if (_selectionPainter!.selectedDate != null && targetElement != CalendarElement.allDayPanel) {
        ...
        if (targetElement == CalendarElement.calendarCell &&
            !_isEnabledRegion(yPosition, selectedDate, _selectedResourceIndex)) {
          return null;
        }
        ...
        selectedAppointments = <dynamic>[
          CalendarViewHelper.getAppointmentDetail(selectedAppointment, widget.calendar.dataSource)
        ];
  ...
  bool _isEnabledRegion(double y, DateTime? selectedDate, int resourceIndex) {
    ...
    final double minuteHeight =
        timeIntervalSize / CalendarViewHelper.getTimeInterval(widget.calendar.timeSlotViewSettings);
    final Duration startDuration = Duration(
        hours: widget.calendar.timeSlotViewSettings.startHour.toInt(),
        minutes:
            ((widget.calendar.timeSlotViewSettings.startHour - widget.calendar.timeSlotViewSettings.startHour.toInt()) *
                    60)
                .toInt());
    int minutes;
    if (CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if (_isRTL) {
        minutes = ((_scrollController!.offset + (_scrollController!.position.viewportDimension - y)) % viewWidth) ~/
            minuteHeight;
      } else {
    ...
    final DateTime date =
        DateTime(selectedDate.year, selectedDate.month, selectedDate.day, 0, minutes + startDuration.inMinutes);
  ...
  double _getTimeIntervalHeight(SfCalendar calendar, CalendarView view, double width, double height,
      int visibleDatesCount, bool isMobilePlatform) {
    final bool isTimelineView = CalendarViewHelper.isTimelineView(view);
    final bool isDayView = CalendarViewHelper.isDayView(view, calendar.timeSlotViewSettings.numberOfDaysInView,
        calendar.timeSlotViewSettings.nonWorkingDays, calendar.monthViewSettings.numberOfWeeksInView);
    ...
    if (isDayView) {
      ...
      allDayViewHeight = _updateCalendarStateDetails.allDayPanelHeight > _kAllDayLayoutHeight
          ? _kAllDayLayoutHeight
          : _updateCalendarStateDetails.allDayPanelHeight;
    }
  ...
  DateTime? _getTappedViewHeaderDate(Offset localPosition, double width) {
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      index = ((_scrollController!.offset +
                  (_isRTL ? _scrollController!.position.viewportDimension - localPosition.dx : localPosition.dx)) /
  ...
  void _updateHoveringForViewHeader(Offset localPosition, double xPosition, double yPosition, double viewHeaderHeight) {
    if (widget.calendar.onTap == null && widget.calendar.onLongPress == null) {
      final bool isViewNavigationEnabled = widget.calendar.allowViewNavigation &&
          widget.view != CalendarView.month &&
          widget.view != CalendarView.day &&
          widget.view != CalendarView.timelineDay;
    ...
    final DateTime? hoverDate = _getTappedViewHeaderDate(
        Offset(CalendarViewHelper.isTimelineView(widget.view) ? localPosition.dx : xPosition, yPosition), widget.width);
    ...
    if (widget.view == CalendarView.timelineMonth &&
        CalendarViewHelper.isDateInDateCollection(widget.blackoutDates, hoverDate)) {
      _removeViewHeaderHovering();
  ...
  void _updateMouseCursorForAppointment(
      AppointmentView? appointmentView, double xPosition, double yPosition, bool isTimelineViews,
    ...
    if (isAllDayPanel || (widget.view == CalendarView.month || isTimelineViews)) {
      ...
      if (xPosition >= appointmentView.appointmentRect!.left &&
          ...
          ((isMonthView &&
                  isSameDate(_isRTL ? appointmentEndTime : appointmentStartTime,
                      _isRTL ? appointmentExactEndTime : appointmentExactStartTime)) ||
              (!isMonthView &&
                  CalendarViewHelper.isSameTimeSlot(_isRTL ? appointmentEndTime : appointmentStartTime,
                      _isRTL ? appointmentExactEndTime : appointmentExactStartTime))) &&
        ...
        setState(() {
          ...
          ((isMonthView &&
                  isSameDate(_isRTL ? appointmentStartTime : appointmentEndTime,
                      _isRTL ? appointmentExactStartTime : appointmentExactEndTime)) ||
              (!isMonthView &&
                  CalendarViewHelper.isSameTimeSlot(_isRTL ? appointmentStartTime : appointmentEndTime,
                      _isRTL ? appointmentExactStartTime : appointmentExactEndTime))) &&
      ...
      if (yPosition >= appointmentView.appointmentRect!.top &&
          ...
          CalendarViewHelper.isSameTimeSlot(
              appointmentView.appointment!.actualStartTime, appointmentView.appointment!.exactStartTime)) {
        setState(() {
          ...
          CalendarViewHelper.isSameTimeSlot(
              appointmentView.appointment!.actualEndTime, appointmentView.appointment!.exactEndTime)) {
        setState(() {
  ...
  void _updatePointerHover(Offset globalPosition) {
    ...
    if (widget.view != CalendarView.month && !isTimelineViews) {
        ...
        if (isDayView) {
          if ((_isRTL && localPosition.dx < widget.width - timeLabelWidth) ||
            ...
            final AppointmentView? appointment = _getAllDayAppointmentOnPoint(
                _updateCalendarStateDetails.allDayAppointmentViewCollection, localPosition.dx, localPosition.dy);
            _updateMouseCursorForAppointment(appointment, localPosition.dx, localPosition.dy, isTimelineViews,
                isAllDayPanel: true);
            return;
          }

          _updateHoveringForViewHeader(localPosition, _isRTL ? widget.width - localPosition.dx : localPosition.dx,
      ...
      final double allDayBottom =
          isDayView ? viewHeaderHeight : viewHeaderHeight + _allDayHeight + allDayExpanderHeight;
      ...
      final AppointmentView? appointment =
          _appointmentLayout.getAppointmentViewOnPoint(localPosition.dx, yPosition + _scrollController!.offset);
      _hoveringAppointmentView = appointment;
      if (appointment != null) {
        _updateHoveringForAppointment(localPosition.dx, yPosition + _scrollController!.offset);
        _updateMouseCursorForAppointment(
            appointment, localPosition.dx, yPosition + _scrollController!.offset, isTimelineViews);
        _hoveringDate = null;
        return;
      }
    } else {
      ...
      if (isTimelineViews) {
        if (_isRTL) {
          xPosition = (_getSingleViewWidthForTimeLineView(this) * widget.visibleDates.length) -
    ...
    final DateTime? hoverDate =
        _getDateFromPosition(isTimelineViews ? localPosition.dx : xPosition, yPosition, timeLabelWidth);
    ...
    if (hoverDate == null ||
        ...
        (!isMonthView &&
            !CalendarViewHelper.isDateTimeWithInDateTimeRange(
                widget.calendar.minDate, widget.calendar.maxDate, hoverDate, timeInterval))) {
      if (_hoveringDate != null) {
        _hoveringDate = null;
      }
    ...
    if (isMonthView &&
        !CalendarViewHelper.isCurrentMonthDate(widget.calendar.monthViewSettings.numberOfWeeksInView,
            widget.calendar.monthViewSettings.showTrailingAndLeadingDates, currentMonth, hoverDate)) {
      if (_hoveringDate != null) {
        _hoveringDate = null;
      }
  ...
  DateTime? _getDateFromPositionForTimeline(double cellWidth, double cellHeight, double x, double y) {
    ...
    if (_isRTL) {
      rowIndex = (((_scrollController!.offset % _getSingleViewWidthForTimeLineView(this)) +
      ...
      case CalendarView.month:
        {
          ...
          cellHeight =
              (widget.height - CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view)) /
                  widget.calendar.monthViewSettings.numberOfWeeksInView;
  ...
  _SelectionPainter _addSelectionView([double? resourceItemHeight]) {
    ...
    if (appointmentView != null &&
        _updateCalendarStateDetails.visibleAppointments.contains(appointmentView.appointment)) {
      _selectionPainter!.appointmentView = appointmentView;
    }
...
class _ViewHeaderViewPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double weekNumberPanelWidth =
        CalendarViewHelper.getWeekNumberPanelWidth(showWeekNumber, size.width, isMobilePlatform);
  ...
  void _addViewHeaderForMonthView(Canvas canvas, Size size, TextStyle viewHeaderDayStyle, double width, DateTime today,
      double weekNumberPanelWidth) {
    ...
    for (int i = 0; i < DateTime.daysPerWeek; i++) {
      ...
      String dayText = DateFormat(monthViewSettings.dayFormat, locale).format(currentDate).capitalize();
    ...
    if (weekNumberPanelWidth != 0 && showWeekNumber) {
      ...
      if (_dayTextPainter.didExceedMaxLines &&
          (_dayTextPainter.width <= (weekNumberTextStyle.fontSize ?? defaultFontSize) * 1.5)) {
        _dayTextPainter.ellipsis = null;
        _dayTextPainter.layout(maxWidth: weekNumberPanelWidth);
      }

      _dayTextPainter.paint(
          canvas, Offset(xPosition + (weekNumberPanelWidth / 2 - _dayTextPainter.width / 2), yPosition));
    }
  }

  void _addViewHeaderForTimeSlotViews(Canvas canvas, Size size, TextStyle viewHeaderDayStyle,
      TextStyle viewHeaderDateStyle, double width, DateTime today) {
    double xPosition, yPosition;
    final bool isDayView = CalendarViewHelper.isDayView(view, timeSlotViewSettings.numberOfDaysInView,
        timeSlotViewSettings.nonWorkingDays, monthViewSettings.numberOfWeeksInView);
    final double labelWidth = isDayView && timeLabelWidth < 50 ? 50 : timeLabelWidth;
    TextStyle dayTextStyle = viewHeaderDayStyle;
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      String dayText = DateFormat(timeSlotViewSettings.dayFormat, locale).format(currentDate).capitalize();
      ...
      if (isToday) {
        ...
        dateTextStyle = todayTextStyle != null
            ? calendarTheme.todayTextStyle!.copyWith(fontSize: viewHeaderDateStyle.fontSize)
      ...
      if (isToday) {
        _drawTodayCircle(canvas, xPosition + dateXPosition,
            yPosition + topPadding + _dayTextPainter.height + inBetweenPadding, _dateTextPainter);
      }

      if (viewHeaderNotifier.value != null) {
        _addMouseHoverForTimeSlotView(
            canvas, size, xPosition, yPosition, dateXPosition, topPadding, isToday, inBetweenPadding);
      }

      _dateTextPainter.paint(canvas,
          Offset(xPosition + dateXPosition, yPosition + topPadding + _dayTextPainter.height + inBetweenPadding));
      if (!isDayView &&
          showWeekNumber &&
        ...
        final double weekNumberPosition = isRTL
            ? (size.width - timeLabelWidth) + ((timeLabelWidth - _dateTextPainter.width) / 2)
  ...
  void _addMouseHoverForMonth(Canvas canvas, Size size, double xPosition, double yPosition, double width) {
    if (xPosition + (width / 2 - _dayTextPainter.width / 2) <= viewHeaderNotifier.value!.dx &&
      ...
      _drawTodayCircle(canvas, xPosition + (width / 2 - _dayTextPainter.width / 2), yPosition, _dayTextPainter,
          hoveringColor:
              (calendarTheme.brightness == Brightness.dark ? Colors.white : Colors.black87).withOpacity(0.04));
    }
  }

  void _addMouseHoverForTimeSlotView(Canvas canvas, Size size, double xPosition, double yPosition, double dateXPosition,
      double topPadding, bool isToday, int padding) {
    if (xPosition + dateXPosition <= viewHeaderNotifier.value!.dx &&
        xPosition + dateXPosition + _dateTextPainter.width >= viewHeaderNotifier.value!.dx) {
      final Color hoveringColor = isToday
          ? Colors.black.withOpacity(0.12)
          : (calendarTheme.brightness == Brightness.dark ? Colors.white : Colors.black87).withOpacity(0.04);
      _drawTodayCircle(canvas, xPosition + dateXPosition, yPosition + topPadding + _dayTextPainter.height + padding,
          _dateTextPainter,
  ...
  String _updateViewHeaderFormat(String dayFormat, String dayText) {
    switch (view) {
      ...
      case CalendarView.workWeek:
        {
          if (!CalendarViewHelper.isDayView(view, timeSlotViewSettings.numberOfDaysInView,
                  timeSlotViewSettings.nonWorkingDays, monthViewSettings.numberOfWeeksInView) &&
    ...
    for (int i = 0; i < DateTime.daysPerWeek; i++) {
      semanticsBuilder.add(CustomPainterSemantics(
        ...
        properties: SemanticsProperties(
          label: DateFormat('EEEEE').format(visibleDates[i]).capitalize(),
  ...
  List<CustomPainterSemantics> _getSemanticsForDayHeader(Size size) {
    ...
    for (int i = 0; i < visibleDates.length; i++) {
      ...
      if (showWeekNumber &&
        ...
        semanticsBuilder.add(CustomPainterSemantics(
            rect: Rect.fromLTWH(
                isRTL ? (size.width - timeLabelWidth) : 0, 0, isRTL ? size.width : timeLabelWidth, viewHeaderHeight),
            properties: SemanticsProperties(
...
class _SelectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.clipRect(Rect.fromLTWH(0, 0, size.width, size.height));
    final double timeLabelWidth =
        CalendarViewHelper.getTimeLabelWidth(calendar.timeSlotViewSettings.timeRulerSize, view);
  ...
  void _drawMonthSelection(Canvas canvas, Size size, double width) {
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      if (isSameDate(visibleDates[i], selectedDate)) {
        final double weekNumberPanelWidth =
            CalendarViewHelper.getWeekNumberPanelWidth(showWeekNumber, width, isMobilePlatform);
  ...
  DateTime _updateSelectedDate() {
    ...
    final double startMinute =
        (calendar.timeSlotViewSettings.startHour - calendar.timeSlotViewSettings.startHour.toInt()) * 60;
    ...
    if (isDateWithInDateRange(visibleDates[0], visibleDates[visibleDates.length - 1], selectedDate)) {
      ...
      for (int i = 0; i < visibleDates.length; i++) {
        if (isSameDate(selectedDate, visibleDates[i])) {
          ...
          _xPosition =
              (i * singleViewWidth) + AppointmentHelper.timeToPosition(calendar, selectedDate!, timeIntervalHeight);
          if (isRTL) {
  ...
  void _drawSlotSelection(double width, double height, Canvas canvas) {
    ...
    _boxPainter = selectionDecoration!.createBoxPainter(_updateSelectionDecorationPainter);
    _boxPainter.paint(
        canvas, Offset(rect.left, rect.top), ImageConfiguration(size: rect.size, textDirection: TextDirection.ltr));
  }
...
class _TimeRulerView extends CustomPainter {
  _TimeRulerView(this.horizontalLinesCount, this.timeIntervalHeight, this.timeSlotViewSettings, this.cellBorderColor,
      this.isRTL, this.locale, this.calendarTheme, this.isTimelineView, this.visibleDates, this.textScaleFactor);
  ...
  /// calendar.
  void _drawTimeLabels(Canvas canvas, Size size, DateTime date, double hour, double xPosition, double yPosition,
    ...
    for (int i = isTimelineView ? 0 : 1; i <= (isTimelineView ? horizontalLinesCount - 1 : horizontalLinesCount); i++) {
      ...
      final String time = DateFormat(timeSlotViewSettings.timeFormat, locale).format(date);
...
class _CurrentTimeIndicator extends CustomPainter {
  _CurrentTimeIndicator(this.timeIntervalSize, this.timeRulerSize, this.timeSlotViewSettings, this.isTimelineView,
      this.visibleDates, this.todayHighlightColor, this.isRTL, ValueNotifier<int> repaintNotifier)
      : super(repaint: repaintNotifier);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double currentTimePosition = CalendarViewHelper.getTimeToPosition(
        Duration(hours: hours, minutes: minutes), timeSlotViewSettings, minuteHeight);
...
DateTime? _timeFromPosition(DateTime date, TimeSlotViewSettings timeSlotViewSettings, double positionY,
    _CalendarViewState? currentState, double timeIntervalHeight, bool isTimelineView) {
  ...
  final double singleIntervalHeightForAnHour =
      (60 / CalendarViewHelper.getTimeInterval(timeSlotViewSettings)) * timeIntervalHeight;
  ...
  if (isTimelineView) {
    if (currentState!._isRTL) {
      positionY = (currentState._scrollController!.offset % _getSingleViewWidthForTimeLineView(currentState)) +
          (currentState._scrollController!.position.viewportDimension - positionY);
    } else {
      positionY += currentState._scrollController!.offset % _getSingleViewWidthForTimeLineView(currentState);
    }
  } else {
...
class _ResizingAppointmentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double scrollOffset = view == CalendarView.month || resizingDetails.value.isAllDayPanel
        ? 0
        : resizingDetails.value.scrollPosition ?? scrollController!.offset;

    final bool isForwardResize =
        mouseCursor == SystemMouseCursors.resizeDown || mouseCursor == SystemMouseCursors.resizeRight;
    final bool isBackwardResize =
        mouseCursor == SystemMouseCursors.resizeUp || mouseCursor == SystemMouseCursors.resizeLeft;
    ...
    late Rect rect;
    if (resizingDetails.value.monthRowCount != 0 && view == CalendarView.month) {
        ...
        if (isHorizontalResize) {
          if (resizingDetails.value.isAllDayPanel || view == CalendarView.month) {
            ...
            if (isRTL) {
              ...
              left = left -
                  ((scrollController!.position.viewportDimension + scrollController!.position.maxScrollExtent) -
    ...
    if (view != CalendarView.month) {
      ...
      _addSubjectTextForMonthView(canvas, resizingDetails.value.appointmentView!.appointmentRect!, appointmentTextStyle,
          span, isRecurrenceAppointment, xPosition, rect, yPosition);
    }
  ...
  void _drawTimeIndicator(Canvas canvas, bool isTimelineView, Size size, bool isBackwardResize) {
    ...
    _updateTextPainter(span);
    _textPainter.layout(maxWidth: isTimelineView ? timeIntervalHeight : timeLabelWidth);
    double xPosition;
    double yPosition;
    if (isTimelineView) {
  ...
  void _addSubjectTextForTimeslotViews(Canvas canvas, int textStartPadding, double xPosition, double yPosition,
      bool isRecurrenceAppointment, Rect rect) {
    ...
    _textPainter.paint(canvas, Offset(xPosition + (isRTL ? 0 : textStartPadding), yPosition + textStartPadding));
  ...
  void _addSubjectTextForMonthView(Canvas canvas, RRect appointmentRect, TextStyle style, TextSpan span,
      bool isRecurrenceAppointment, double xPosition, Rect rect, double yPosition) {
    double textSize = -1;
    if (textSize == -1) {
      ...
      for (double j = style.fontSize! - 1; j > 0; j--) {
        ...
        if (_textPainter.height >= appointmentRect.height) {
          ...
          span = TextSpan(text: resizingDetails.value.appointmentView!.appointment!.subject, style: style);
          _updateTextPainter(span);
        } else {
          textSize = j + 1;
          break;
      ...
      span = TextSpan(
          text: resizingDetails.value.appointmentView!.appointment!.subject, style: style.copyWith(fontSize: textSize));
      _updateTextPainter(span);
    }
    final double textWidth = appointmentRect.width - (isRecurrenceAppointment ? textSize : 1);
    _textPainter.layout(maxWidth: textWidth > 0 ? textWidth : 0);
    if (isRTL) {
      xPosition -= (isRTL ? 0 : 2) + _textPainter.width;
    }
    yPosition = yPosition + ((appointmentRect.height - _textPainter.height) / 2);
  ...
  void _addRecurrenceIcon(Rect rect, Canvas canvas, int? textPadding, bool isRecurrenceAppointment, double textSize) {
    ...
    if (view == CalendarView.month) {
      ...
      final double yPosition = rect.top + ((rect.height - _textPainter.height) / 2);
      const double rightPadding = 0;
      final double recurrenceStartPosition =
          isRTL ? rect.left + rightPadding : rect.right - _textPainter.width - rightPadding;
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTRB(
                  recurrenceStartPosition, yPosition, recurrenceStartPosition + _textPainter.width, rect.bottom),
      ...
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTRB(isRTL ? rect.left + textSize + xPadding : rect.right - textSize - xPadding,
                  rect.bottom - bottomPadding - textSize, isRTL ? rect.left : rect.right, rect.bottom),
              resizingDetails.value.appointmentView!.appointmentRect!.tlRadius),
          _shadowPainter);
      _textPainter.paint(
          canvas,
          Offset(
              isRTL ? rect.left + xPadding : rect.right - textSize - xPadding, rect.bottom - bottomPadding - textSize));
    }
  }
          ...
          CalendarAppointmentDetails(
              ...
              List<dynamic>.unmodifiable(<dynamic>[
                CalendarViewHelper.getAppointmentDetail(
                    _draggingAppointmentView!.appointment!, widget.calendar.dataSource)
              ]),
              Rect.fromLTWH(
...
class _DraggingAppointmentRenderObject extends RenderBox with RenderObjectWithChildMixin<RenderBox> {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (child == null) {
      ...
      context.paintChild(
          ...
          Offset(
              isRTL
                  ? dragDetails.position.value!.dx - dragDetails.appointmentView!.appointmentRect!.width
                  : dragDetails.position.value!.dx,
              dragDetails.position.value!.dy));
      if (dragAndDropSettings.showTimeIndicator && dragDetails.draggingTime != null) {
  ...
  void _drawDefaultUI(Canvas canvas, bool isTimelineView) {
    ...
    _shadowPainter.color = dragDetails.appointmentView!.appointment!.color.withOpacity(0.5);
    ...
    _textPainter.paint(
        canvas,
        isTimelineView
            ? Offset(xPosition + (isRTL ? 0 : textStartPadding), yPosition + textStartPadding)
            : Offset(xPosition + (isRTL ? 0 : textStartPadding), yPosition + textStartPadding));
    if (dragAndDropSettings.showTimeIndicator && dragDetails.draggingTime != null) {
  ...
  void _drawTimeIndicator(Canvas canvas, bool isTimelineView, Size size) {
    if (calendarView == CalendarView.month || calendarView == CalendarView.timelineMonth) {
      return;
    }
    ...
    _textPainter.textWidthBasis = TextWidthBasis.longestLine;
    _textPainter.textScaleFactor = textScaleFactor;
    final double timeLabelSize = isTimelineView ? dragDetails.timeIntervalHeight! : timeLabelWidth;
