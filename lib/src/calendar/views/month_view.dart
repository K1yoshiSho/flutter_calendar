    ...
    if (widget.builder != null) {
      ...
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        if (widget.calendar.dataSource != null &&
            !AppointmentHelper.isCalendarAppointment(widget.calendar.dataSource!)) {
          monthCellAppointment = CalendarViewHelper.getCustomAppointments(appointments, widget.calendar.dataSource);
        }
        ...
        final Widget child = widget.builder!(
            ...
            MonthCellDetails(
                ...
                Rect.fromLTWH(widget.isRTL ? widget.width - xPosition - cellWidth : xPosition, yPosition, cellWidth,
                    cellHeight)));
...
class _MonthViewRenderObject extends CustomCalendarRenderObject {
  ...
  void performLayout() {
    ...
    for (dynamic child = firstChild; child != null; child = childAfter(child)) {
      child.layout(
          constraints.copyWith(minWidth: cellWidth, minHeight: cellHeight, maxWidth: cellWidth, maxHeight: cellHeight));
    }
  }
    ...
    if (!isNeedCustomPaint) {
      ...
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        if (currentVisibleDate.weekday == DateTime.monday) {
          ...
          final DateTime startDate = visibleDates[(i ~/ DateTime.daysPerWeek) * DateTime.daysPerWeek];

          /// Calculate the row end date based on visible dates index.
          final DateTime endDate = addDuration(startDate, const Duration(days: DateTime.daysPerWeek - 1)) as DateTime;

          /// Used to check the start and end date is current month date or not.
          final bool isCurrentMonthWeek = startDate.month == currentMonth || endDate.month == currentMonth;

          if (weekNumberPanelWidth != 0 &&
              (showTrailingLeadingDates || (!showTrailingLeadingDates && isCurrentMonthWeek))) {
        ...
        context.paintChild(child!, Offset(isRTL ? size.width - xPosition - cellWidth : xPosition, yPosition));
        child = childAfter(child);

        if (calendarCellNotifier.value != null && !_blackoutDatesIndex.contains(i)) {
  ...
  void _drawWeekNumberPanel(Canvas canvas, double cellHeight) {
    ...
    final RRect roundedRect = RRect.fromRectAndRadius(rect, Radius.circular(padding));
  ...
  void _drawMonthCells(Canvas canvas, Size size) {
    ...
    final TextStyle previousMonthTextStyle = calendarTheme.trailingDatesTextStyle!;
    ...
    final Color currentMonthBackgroundColor =
        monthCellStyle.backgroundColor ?? calendarTheme.activeDatesBackgroundColor!;
    ...
    for (int i = 0; i < visibleDatesCount; i++) {
      ...
      if (currentVisibleDate.weekday == DateTime.monday) {
        ...
        final bool isCurrentMonthWeek =
            startDate.month == currentMonthDate.month || endDate.month == currentMonthDate.month;
      ...
      if (calendarCellNotifier.value != null && !isBlackoutDate) {
        _addMouseHovering(canvas, size, cellWidth, cellHeight, xPosition, yPosition - viewPadding);
      }
      ...
      if (isCurrentDate) {
        ...
        canvas.drawCircle(Offset(xPosition + cellWidth / 2, yPosition + circlePadding + textHeight),
            textHeight + viewPadding, _linePainter);
      }

      _textPainter.paint(
          canvas, Offset(xPosition + (cellWidth / 2 - _textPainter.width / 2), yPosition + circlePadding));
  ...
  void _addMouseHovering(
      Canvas canvas, Size size, double cellWidth, double cellHeight, double xPosition, double yPosition) {
    if (xPosition <= calendarCellNotifier.value!.dx &&
      ...
      canvas.drawRect(
          Rect.fromLTWH(
              ...
              yPosition,
              (xPosition + cellWidth).round() >= size.width ? cellWidth - linePadding - 1 : cellWidth - 1,
              (yPosition + cellHeight).round() >= size.height.round() ? cellHeight - 1 - linePadding : cellHeight - 1),
          _linePainter);
    }
  }
  ...
  void _drawVerticalAndHorizontalLines(
    ...
    final double finalXPosition = isRTL ? size.width - weekNumberPanelWidth : size.width;
    ...
    xPosition = weekNumberPanelWidth != 0 && !isRTL ? weekNumberPanelWidth : cellWidth;
  ...
  String _getAccessibilityText(DateTime date, int index) {
    final String accessibilityText = DateFormat('EEE, dd MMMM yyyy').format(date);
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    final double cellWidth = (size.width - weekNumberPanelWidth) / DateTime.daysPerWeek;
    double left = isRTL ? size.width - cellWidth - weekNumberPanelWidth : weekNumberPanelWidth, top = 0;
    final double cellHeight = size.height / rowCount;
    final bool showTrailingLeadingDates =
        CalendarViewHelper.isLeadingAndTrailingDatesVisible(rowCount, showTrailingAndLeadingDates);
    ...
    for (int i = 0; i < visibleDates.length; i++) {
      ...
      if (currentVisibleDate.weekday == DateTime.monday) {
        ...
        if (weekNumberPanelWidth != 0 &&
          ...
          semanticsBuilder.add(CustomPainterSemantics(
              rect: Rect.fromLTWH(isRTL ? (size.width - left - cellWidth) : 0, top, weekNumberPanelWidth, cellHeight),
      ...
      if (showTrailingLeadingDates || currentMonth == currentVisibleDate.month) {
        semanticsBuilder.add(CustomPainterSemantics(
          rect: Rect.fromLTWH(isRTL ? size.width - left - cellWidth : left, top, cellWidth, cellHeight),
          properties: SemanticsProperties(
