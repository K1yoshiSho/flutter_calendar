...
class RecurrenceHelper {
  ...
  static bool _isRecurrenceInBetweenSpecificRange(
      DateTime appointmentDate, Duration duration, DateTime visibleStartDate, DateTime visibleEndTime) {
    final DateTime appointmentEndDate = DateTimeHelper.getDateTimeValue(addDuration(appointmentDate, duration));

    /// ignore: lines_longer_than_80_chars
    return isDateWithInDateRange(visibleStartDate, visibleEndTime, appointmentDate) ||
        isDateWithInDateRange(visibleStartDate, visibleEndTime, appointmentEndDate) ||
        isDateWithInDateRange(appointmentDate, appointmentEndDate, visibleStartDate);
  }

  static List<DateTime> _getDailyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
      {Duration? recurrenceDuration, DateTime? specificStartDate, DateTime? specificEndDate}) {
    final List<DateTime> recDateCollection = <DateTime>[];

    if (specificEndDate != null) {
      specificEndDate = DateTime(specificEndDate.year, specificEndDate.month, specificEndDate.day, 23, 59, 59);
    }

    recurrenceDuration ??= Duration.zero;
    final bool isSpecificDateRange = specificStartDate != null && specificEndDate != null;
    ...
    final int dailyDayGap = rRule.contains('INTERVAL') ? int.parse(intervalCountString) : 1;
    DateTime? endDate;
    if (rRule.contains('UNTIL')) {
      ...
      if (isSpecificDateRange) {
        final DateTime startTime = DateTime(endDate.year, endDate.month, endDate.day, recurrenceStartHour,
        ...
        if (specificStartDate.isAfter(endTime) && !isSameDate(specificStartDate, endTime)) {
          return recDateCollection;
        }
      }
      ...
      endDate = AppointmentHelper.addDaysWithTime(recurrenceStartDate, (recurrenceCount - 1) * dailyDayGap,
    ...
    if (isSpecificDateRange) {
      endDate = endDate == null || endDate.isAfter(specificEndDate) ? specificEndDate : endDate;
    }
    ...
    if (isSpecificDateRange && recurrenceStartDate.isBefore(specificStartDate)) {
      final DateTime recurrenceInitialDate =
          DateTime(recurrenceStartDate.year, recurrenceStartDate.month, recurrenceStartDate.day);
      final DateTime visibleInitialDate =
          DateTime(specificStartDate.year, specificStartDate.month, specificStartDate.day);

      /// Total days difference between the visible start date and recurrence
      /// start date.
      final int difference = AppointmentHelper.getDifference(recurrenceInitialDate, visibleInitialDate).inDays;
      ...
      if (dayDifference == 0) {
        addDate = DateTime(specificStartDate.year, specificStartDate.month, specificStartDate.day, recurrenceStartHour,
            recurrenceStartMinute, recurrenceStartSecond);
      } else {
        ...
        addDate = AppointmentHelper.addDaysWithTime(
            visibleInitialDate, -dayDifference, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      }
      ...
      if (incrementCount > 0 && !isSameDate(addDate, recurrenceEndDate)) {
        final int durationDifference =
            recurrenceEndDate.hour > addDate.hour ? recurrenceDuration.inDays : recurrenceDuration.inDays + 1;
        final int intervalCount = ((durationDifference ~/ dailyDayGap) * dailyDayGap) +
            (durationDifference % dailyDayGap == 0 ? 0 : dailyDayGap);
        addDate = AppointmentHelper.addDaysWithTime(
            addDate, -intervalCount, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
    ...
    while (recurrenceIncrementCount < recurrenceCount ||
        (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
      if (isSpecificDateRange) {
        if (_isRecurrenceInBetweenSpecificRange(addDate, recurrenceDuration, specificStartDate, specificEndDate)) {
      ...
      addDate = AppointmentHelper.addDaysWithTime(
          addDate, dailyDayGap, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
  ...
  static List<DateTime> _getWeeklyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
      {Duration? recurrenceDuration, DateTime? specificStartDate, DateTime? specificEndDate}) {
    final List<DateTime> recDateCollection = <DateTime>[];
    ...
    const List<String> weekDaysString = <String>['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'];
    final List<String> ruleArray = splitRule(rRule, ruleSeparator);
    if (ruleArray.isEmpty) {
      return recDateCollection;
    }
    ...
    final int weeklyByDayPos = weeklyByDayRules.isNotEmpty ? int.parse(weeklyByDayRules[1]) : -1;

    final int recurrenceStartHour = recurrenceStartDate.hour;
    final int recurrenceStartMinute = recurrenceStartDate.minute;
    final int recurrenceStartSecond = recurrenceStartDate.second;
    final int recurrenceCount = recurrenceCountString.isNotEmpty ? int.parse(recurrenceCountString) : 0;

    int tempCount = 0;
    final int weeklyWeekGap = ruleArray.length > 4 && rRule.contains('INTERVAL') ? int.parse(intervalCountString) : 1;
    ...
    if (rRule.contains('UNTIL')) {
      ...
      totalDays += DateTime.daysPerWeek - weekDay + (DateTime.daysPerWeek * (weeklyWeekGap - 1));
      ...
      if (remainingCount != 0 && tempRecurrenceCount > 0) {
        ...
        while (additionalDays < DateTime.daysPerWeek && currentRecurrenceCount != 0) {
        ...
        totalDays -= additionalDays + (DateTime.daysPerWeek * (weeklyWeekGap - 1));
      }

      endDate = AppointmentHelper.addDaysWithTime(
          recurrenceStartDate, totalDays, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
    ...
    DateTime addDate = recurrenceStartDate;
    if (isSpecificDateRange && recurrenceStartDate.isBefore(specificStartDate)) {
      final DateTime startDate = DateTime(addDate.year, addDate.month, addDate.day);

      /// Calculate the total days between the recurrence start and visible
      /// start date.
      int daysDifference = AppointmentHelper.getDifference(startDate, specificStartDate).inDays;
      final DateTime recurrenceEndDate = addDate.add(recurrenceDuration);
      ...
      while (initialWeekDay < DateTime.daysPerWeek && daysDifference > initialWeekDaysCount) {
        if (weekDays.contains(initialWeekDay)) {
          tempRecurrenceCount++;
        }
      ...
      final bool isOccurrenceInInitialWeek = initialWeekDaysCount + weekDay < DateTime.daysPerWeek;
      ...
      final int initialWeekGap = isOccurrenceInInitialWeek ? 0 : DateTime.daysPerWeek * (weeklyWeekGap - 1);
      ...
      final int totalWeeks = daysDifference ~/ (DateTime.daysPerWeek * weeklyWeekGap);
      ...
      addDate = AppointmentHelper.addDaysWithTime(
          ...
          (totalWeeks * DateTime.daysPerWeek * weeklyWeekGap) +
              (isOccurrenceInInitialWeek ? daysDifference : DateTime.daysPerWeek - weekDay) +
              initialWeekGap,
          recurrenceStartHour,
    ...
    while ((tempCount < recurrenceCount && isWeeklySelected) ||
        (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
      final bool isRecurrenceDate = weekDays.contains(addDate.weekday % DateTime.daysPerWeek);
      if (isSpecificDateRange) {
        if (_isRecurrenceInBetweenSpecificRange(addDate, recurrenceDuration, specificStartDate, specificEndDate) &&
            isRecurrenceDate) {
          recDateCollection.add(addDate);
      ...
      addDate = addDate.weekday == DateTime.saturday
          ? AppointmentHelper.addDaysWithTime(addDate, ((weeklyWeekGap - 1) * DateTime.daysPerWeek) + 1,
              recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond)
          : AppointmentHelper.addDaysWithTime(
              addDate, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
    }

    return recDateCollection;
  }

  static List<DateTime> _getMonthlyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
      {Duration? recurrenceDuration, DateTime? specificStartDate, DateTime? specificEndDate}) {
    final List<DateTime> recDateCollection = <DateTime>[];
    ...
    final int monthlyMonthGap = ruleArray.length > 4 && intervalCount.isNotEmpty ? int.parse(intervalCount) : 1;
    DateTime? endDate;
    if (rRule.contains('UNTIL')) {
    ...
    if (isSpecificDateRange && !rRule.contains('COUNT')) {
      ...
      if (addedDateYear < viewStartDateYear ||
          (viewStartDateMonth >= addedDateMonth && viewStartDateYear == addedDateYear)) {
        /// Calculate the total months between the recurrence start date and
        /// visible start date.
        final int totalMonths = (viewStartDateMonth - addedDateMonth) + ((viewStartDateYear - addedDateYear) * 12);
        ...
        addDate = DateTime(addedDateYear, addedDateMonth + (validMonths * monthlyMonthGap));
        if (addDate.isBefore(recurrenceStartDate)) {
          addDate = recurrenceStartDate;
        }
      }
    ...
    if (byMonthDay == 'BYMONTHDAY') {
      ...
      final DateTime temp = DateTime(
          addDate.year, addDate.month, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      if (temp.day == monthDate && (temp.isAfter(recurrenceStartDate) || isSameDate(temp, recurrenceStartDate))) {
        addDate = temp;
      } else {
        ...
        addDate = DateTime(addDate.year, addDate.month + monthlyMonthGap, 1, recurrenceStartHour, recurrenceStartMinute,
            recurrenceStartSecond);
      ...
      int tempCount = 0;
      while (tempCount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
        if (addDate.day != monthDate) {
          ...
          addDate = DateTime(
              yearValue, monthValue, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          continue;
        }
        ...
        monthValue += monthlyMonthGap;
        monthDate =
            byMonthDay == -1 ? AppointmentHelper.getMonthEndDate(DateTime(yearValue, monthValue)).day : monthDate;
        addDate = DateTime(
            yearValue, monthValue, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      void updateValidDate() {
        final DateTime monthStart =
            DateTime(addDate.year, addDate.month, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        ...
        final DateTime weekStartDate = AppointmentHelper.addDaysWithTime(
            monthStart, -monthStartWeekday, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        ...
        if (bySetPosValue.isNegative) {
          addDate = _getRecurrenceDateForNegativeValue(bySetPosValue, monthStart, nthWeekDay);
        } else {
          addDate = AppointmentHelper.addDaysWithTime(weekStartDate, (nthWeek * DateTime.daysPerWeek) + nthWeekDay,
              recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        }
      }
  ...
  static List<DateTime> _getYearlyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
      {Duration? recurrenceDuration, DateTime? specificStartDate, DateTime? specificEndDate}) {
    final List<DateTime> recDateCollection = <DateTime>[];
    ...
    final int yearlyYearGap = ruleArray.length > 4 && intervalCount.isNotEmpty ? int.parse(intervalCount) : 1;
    ...
    if (isSpecificDateRange && !rRule.contains('COUNT')) {
      ...
      if (addedDateYear < viewStartDateYear) {
        ...
        /// and visible start date.
        final int inBetweenYears = (viewStartDateYear - addedDateYear) ~/ yearlyYearGap;
    ...
    if (byMonthDay == 'BYMONTHDAY') {
      ...
      if (byMonthDay == -1) {
        dayIndex = AppointmentHelper.getMonthEndDate(DateTime(addDate.year, monthIndex)).day;
      }
      ...
      final int daysInMonth =
          DateTimeHelper.getDateTimeValue(addDays(DateTime(addDate.year, addDate.month + 1), -1)).day;
      if (daysInMonth < dayIndex) {
        return recDateCollection;
      }

      final DateTime specificDate = DateTime(
          addDate.year, monthIndex, dayIndex, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      if (specificDate.isBefore(recurrenceStartDate)) {
        addDate = DateTime(specificDate.year + yearlyYearGap, specificDate.month, specificDate.day, recurrenceStartHour,
            recurrenceStartMinute, recurrenceStartSecond);
      } else {
      ...
      while (tempCount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
        ...
        dayIndex = byMonthDay == -1
            ? AppointmentHelper.getMonthEndDate(DateTime(addDate.year + yearlyYearGap, monthIndex)).day
            : addDate.day;

        addDate = DateTime(addDate.year + yearlyYearGap, addDate.month, dayIndex, recurrenceStartHour,
            recurrenceStartMinute, recurrenceStartSecond);
        tempCount++;
      }
    } else if (byDay == 'BYDAY') {
      ...
      void updateValidNextDate() {
        while (true) {
          DateTime monthStart =
              DateTime(addDate.year, monthIndex, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          final int monthStartWeekday = monthStart.weekday % DateTime.daysPerWeek;
          final DateTime weekStartDate = AppointmentHelper.addDaysWithTime(
              monthStart, -monthStartWeekday, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          int nthWeek = bySetPosValue;
          ...
          if (bySetPosValue.isNegative) {
            monthStart = _getRecurrenceDateForNegativeValue(bySetPosValue, monthStart, nthWeekDay);
          } else {
            monthStart = AppointmentHelper.addDaysWithTime(weekStartDate, (nthWeek * DateTime.daysPerWeek) + nthWeekDay,
                recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          }

          if (monthStart.month != monthIndex || monthStart.isBefore(recurrenceStartDate)) {
            addDate = DateTime(monthStart.year + yearlyYearGap, monthStart.month, monthStart.day, recurrenceStartHour,
                recurrenceStartMinute, recurrenceStartSecond);
            continue;
          }
      ...
      while (tempCount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
        ...
        addDate = DateTime(addDate.year + yearlyYearGap, addDate.month, addDate.day, recurrenceStartHour,
            recurrenceStartMinute, recurrenceStartSecond);
  ...
  static RecurrenceProperties parseRRule(String rRule, DateTime recStartDate) {
    final RecurrenceProperties recProp = RecurrenceProperties(startDate: recStartDate);
    ...
    final int weeklyByDayPos = weeklyRules.isNotEmpty ? int.parse(weeklyRules[1]) : -1;

    if (rRule.contains('COUNT')) {
      recProp.recurrenceRange = RecurrenceRange.count;
      recProp.recurrenceCount = recurCount.isNotEmpty ? int.parse(recurCount) : 0;
    ...
    if (daily == 'DAILY') {
      ...
      if (byMonthDay == 'BYMONTHDAY') {
        recProp.week = 0;
        recProp.dayOfMonth = byMonthDayCount.isNotEmpty ? int.parse(byMonthDayCount) : 1;
      } else if (byDay == 'BYDAY') {
  ...
  static String _getRRuleForDaily(int recCount, RecurrenceProperties recurrenceProperties, DateTime startDate,
      DateTime? endDate, bool isValidRecurrence, Duration diffTimeSpan) {
    String rRule = '';
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      } else if (recurrenceProperties.recurrenceRange == RecurrenceRange.endDate) {
  ...
  static String _getRRuleForWeekly(DateTime startDate, DateTime? endDate, int recCount,
      RecurrenceProperties recurrenceProperties, bool isValidRecurrence, Duration diffTimeSpan, DateTime prevDate) {
    String rRule = '';
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      int count = 0;
      int i = 0;
      while ((count < recCount && isValidRecurrence && recurrenceProperties.weekDays.isNotEmpty) ||
          (recurrenceProperties.recurrenceRange == RecurrenceRange.endDate &&
              (addDate.isBefore(endDate!) || isSameDate(addDate, endDate))) ||
          (recurrenceProperties.recurrenceRange == RecurrenceRange.noEndDate && i < DateTime.daysPerWeek)) {
        ...
        if (dayKey.isNotEmpty) {
          if (count != 0) {
            final Duration tempTimeSpan = AppointmentHelper.getDifference(prevDate, addDate);
            if (tempTimeSpan <= diffTimeSpan) {
              ...
              if (addDate.weekday != DateTime.saturday) {
                byDay =
                    byDay.isNotEmpty && byDay.substring(byDay.length - 1) == 'A' ? '$byDay,$dayKey' : byDay + dayKey;
              } else {
                byDay = byDay + dayKey;
              }
            ...
            prevDate = addDate;
            count++;
            byDay = byDay.isNotEmpty && byDay.substring(byDay.length - 1) == 'A' ? '$byDay,$dayKey' : byDay + dayKey;
            dayCount++;
          }
        ...
        addDate = addDate.weekday == DateTime.saturday
            ? AppointmentHelper.addDaysWithTime(
                addDate,
                ((recurrenceProperties.interval - 1) * DateTime.daysPerWeek) + 1,
                startDate.hour,
                startDate.minute,
                startDate.second)
            : AppointmentHelper.addDaysWithTime(addDate, 1, startDate.hour, startDate.minute, startDate.second);
        i = i + 1;
      }
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      if (recurrenceProperties.week == 0) {
        ...
        final DateTime firstDate =
            DateTimeHelper.getDateTimeValue(addDays(DateTime.now(), -(DateTime.now().weekday - 1)));
        final List<String> dayNames = List<int>.generate(DateTime.daysPerWeek, (int index) => index)
            .map((int value) => DateFormat(DateFormat.ABBR_WEEKDAY).format(addDays(firstDate, value)))
            .toList();
      ...
      if (AppointmentHelper.getDifference(
              startDate,
              DateTime(startDate.year, startDate.month + recurrenceProperties.interval, startDate.day, startDate.hour,
  ...
  static String _getRRuleForYearly(int recCount, RecurrenceProperties recurrenceProperties, DateTime startDate,
      DateTime? endDate, Duration diffTimeSpan, bool isValidRecurrence) {
    String rRule = '';
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
        recurrenceProperties.recurrenceRange == RecurrenceRange.noEndDate ||
        (recurrenceProperties.recurrenceRange == RecurrenceRange.endDate &&
      ...
      if (AppointmentHelper.getDifference(
              startDate,
              DateTime(startDate.year + recurrenceProperties.interval, startDate.month, startDate.day, startDate.hour,
                  startDate.minute, startDate.second)) <
          diffTimeSpan) {
        isValidRecurrence = false;
      }
  ...
  static String generateRRule(RecurrenceProperties recurrenceProperties, DateTime appStartTime, DateTime appEndTime) {
    ...
    final DateTime startDate = appStartTime.isAfter(recPropStartDate) ? appStartTime : recPropStartDate;
    final DateTime? endDate = recPropEndDate;
    final Duration diffTimeSpan = AppointmentHelper.getDifference(appStartTime, appEndTime);
    int recCount = 0;
    ...
    switch (recurrenceProperties.recurrenceType) {
      case RecurrenceType.daily:
        return _getRRuleForDaily(recCount, recurrenceProperties, startDate, endDate, isValidRecurrence, diffTimeSpan);
      case RecurrenceType.weekly:
        return _getRRuleForWeekly(
            startDate, endDate, recCount, recurrenceProperties, isValidRecurrence, diffTimeSpan, prevDate);
      case RecurrenceType.monthly:
        return _getRRuleForMonthly(recCount, recurrenceProperties, startDate, endDate, diffTimeSpan, isValidRecurrence);
      case RecurrenceType.yearly:
        return _getRRuleForYearly(recCount, recurrenceProperties, startDate, endDate, diffTimeSpan, isValidRecurrence);
    }
  }
  ...
  static int _getWeekDay(String weekDay) {
    int index = 1;
    final DateTime firstDate = DateTimeHelper.getDateTimeValue(addDays(DateTime.now(), -(DateTime.now().weekday - 1)));
    final List<String> dayNames = List<int>.generate(DateTime.daysPerWeek, (int index) => index)
        .map((int value) => DateFormat(DateFormat.ABBR_WEEKDAY).format(addDays(firstDate, value)))
        .toList();
    for (int i = 0; i < DateTime.daysPerWeek; i++) {
      ...
      if (dayName.toUpperCase() == weekDay ||
          weekDay.length == 2 && dayName.substring(0, dayName.length - 1).toUpperCase() == weekDay) {
        index = i;
      }
    }
  ...
  static DateTime getUntilEndDate(String untilString) {
    ...
    if (untilString.contains('T')) {
      ...
      final DateTime endDate =
          untilString.contains('Z') ? DateTime.parse(untilString).toLocal() : DateTime.parse(untilString);
      return endDate;
    } else {
  ...
  static DateTime _getRecurrenceDateForNegativeValue(int bySetPosCount, DateTime date, int weekDay) {
    DateTime? lastDate;
    if (bySetPosCount == -1) {
      lastDate = AppointmentHelper.getMonthEndDate(date);
    } else if (bySetPosCount == -2) {
      lastDate = AppointmentHelper.getMonthEndDate(date).subtract(const Duration(days: DateTime.daysPerWeek));
    }
    ...
    return _getLastWeekDay(
        DateTime(lastDate.year, lastDate.month, lastDate.day, date.hour, date.minute, date.second), weekDay);
  }
  ...
  static String _sortByDay(String byDay) {
    final List<String> sortedDays = <String>['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'];
