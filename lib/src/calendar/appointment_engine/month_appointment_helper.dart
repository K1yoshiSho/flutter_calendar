...
class MonthAppointmentHelper {
  static void _createVisibleAppointments(List<AppointmentView> appointmentCollection,
      List<CalendarAppointment> visibleAppointments, List<DateTime> visibleDates, int startIndex, int endIndex) {
    ...
    for (int i = 0; i < visibleAppointments.length; i++) {
      ...
      if (!appointment.isSpanned &&
          ...
          appointment.actualStartTime.month == appointment.actualEndTime.month) {
        final AppointmentView appointmentView = _createAppointmentView(appointmentCollection);
        appointmentView.appointment = appointment;
        appointmentView.canReuse = false;
        appointmentView.startIndex = getDateIndex(appointment.actualStartTime, visibleDates);
        ...
        if (appointmentView.startIndex == -1 || appointmentView.startIndex < startIndex) {
          appointmentView.startIndex = startIndex;
        }

        appointmentView.endIndex = getDateIndex(appointment.actualEndTime, visibleDates);
        ...
        /// In Sep month view visible start and end index as 2, 31 but
        /// appointment start and end index as 1, 3
        if (appointmentView.endIndex == -1 || appointmentView.endIndex > endIndex) {
        ...
        _createAppointmentInfoForSpannedAppointment(appointmentView, appointmentCollection);
      }
    }
  }

  static void _createAppointmentInfoForSpannedAppointment(
      AppointmentView appointmentView, List<AppointmentView> appointmentCollection) {
    if (appointmentView.startIndex ~/ DateTime.daysPerWeek != appointmentView.endIndex ~/ DateTime.daysPerWeek) {
      final int endIndex = appointmentView.endIndex;
      appointmentView.endIndex =
          (((appointmentView.startIndex ~/ DateTime.daysPerWeek) + 1) * DateTime.daysPerWeek) - 1;
      appointmentView.isSpanned = true;
      if (!appointmentCollection.contains(appointmentView)) {
        appointmentCollection.add(appointmentView);
      }

      final AppointmentView appointmentView1 = _createAppointmentView(appointmentCollection);
      ...
      _createAppointmentInfoForSpannedAppointment(appointmentView1, appointmentCollection);
    } else {
      appointmentView.isSpanned = true;
      if (!appointmentCollection.contains(appointmentView)) {
        appointmentCollection.add(appointmentView);
  ...
  static void _setAppointmentPosition(
      List<AppointmentView> appointmentViewCollection, AppointmentView appointmentView, int viewIndex) {
  ...
  static bool _isInterceptAppointments(AppointmentView appointmentView1, AppointmentView appointmentView2) {
  ...
  static int _orderAppointmentViewBySpanned(AppointmentView appointmentView1, AppointmentView appointmentView2) {
    if (appointmentView1.appointment == null || appointmentView2.appointment == null) {
      return 0;
    }
    ...
    return AppointmentHelper.getDifference(endTime2, startTime2)
        .inMinutes
        .abs()
        .compareTo(AppointmentHelper.getDifference(endTime1, startTime1).inMinutes.abs());
  }

  static void _updateAppointmentPosition(
      List<AppointmentView> appointmentCollection, Map<int, List<AppointmentView>> indexAppointments) {
    ...
    for (int j = 0; j < appointmentCollection.length; j++) {
      ...
      for (int i = appointmentView.startIndex; i <= appointmentView.endIndex; i++) {
        ...
        final List<AppointmentView>? existingAppointments = indexAppointments[i];
  ...
  static int getDateIndex(DateTime date, List<DateTime> visibleDates) {
    ...
    int row = 0;
    for (int i = count - DateTime.daysPerWeek; i >= 0; i -= DateTime.daysPerWeek) {
    ...
    final DateTime endDateTime = DateTimeHelper.getDateTimeValue(addDays(dateTime, 6));
  ...
  static AppointmentView _createAppointmentView(List<AppointmentView> appointmentCollection) {
  ...
  static void updateAppointmentDetails(
      ...
      int startIndex,
      int endIndex) {
    _createVisibleAppointments(appointmentCollection, visibleAppointments, visibleDates, startIndex, endIndex);
